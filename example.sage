...
def CRT(a1,H1,a2,H2,a3,H3,prec):
    ...
    aa,nq = com1(vv1,q1,vv2,q2,vv3,q3,2*prec)
...
def crth(vv1,q1,vv2,q2,prec):
    RR.<T> = PowerSeriesRing(bF, default_prec = 2*prec+1)
    ...
    for i in range(len(LL0)):
    	aa0.append((LL0[i].subs(T=T).O(2*prec)).truncate(2*prec))
    ...
    for i in range(len(LL1)):
    	aa1.append((LL1[i].subs(T=T).O(2*prec)).truncate(2*prec))
    ...
    for i in range(len(Lq)):
    	aq.append((Lq[i].subs(T=T).O(2*prec)).truncate(2*prec))
...
def gmm(q1,q2,prec):
    ...
    a1 = []
    for i in range(len(L1)): 
        a1.append((L1[i].subs(T=T).O(2*prec)).truncate(2*prec))
    ...
    a2 = []
    for i in range(len(L2)):
    	a2.append((L2[i].subs(T=T).O(2*prec)).truncate(2*prec))
