...
\maketitle
\footnotetext[1]{Department of Applied and Computational Mathematics and Statistics, University of Notre Dame, USA}
...
\smallskip\noindent{\bf keywords}: Polynomial system solving, Homotopy, Symbolic Computation, Determinantal algebraic sets, Complexity
...
$\KK[X_1,\dots,X_n]$, we will write $V_r(\mF)=V(I_r(\mF))$. Thus, for
...
$g_1, \ldots, g_s$).  Our main problem is then the following.
...
\begin{example}\label{ex:2}
  ...
  ideal, so Problems~\ref{problem2} and~\ref{problem3} admit the same
  answer, with 7 points described by means of the univariate
...
expected codimension); we are not aware of further generalizations
focusing on isolated points and taking into account multiplicities as
is done in Theorem~\ref{theo:1}.  Previous work by Miller and
...
dimension zero. In this context, Spaenlehauer also gave in~\cite{Spa14} an
...
$\VpFG{p}{\mF}{\mG}=V(I_p(\mF) + \langle g_1,\dots,g_s \rangle)$. Our
...
in respectively $c,\sigma$ and $c',\sigma$: they respectively become
...
$V(\bC)$, we need a suitable deformation of these equations.  We let $T$ be a new
variable, and we define the matrix
...
together with polynomials $\bV=(v_1,\dots,v_s)$ of the form
...
this section to $\bC$ as defined above, but the algorithm accepts
arbitrary inputs. Given a point $\bx$ with coordinates in $\LL$ that belongs to the
zero-set $V(\bC)\subset \LLbar{}^n$, we discuss here how to decide
whether $\bx$ is an isolated point in $V(\bC)$.  Without loss of
generality, we assume that $m\ge n$ (otherwise, $\bx$ cannot be an
isolated solution).
...
unmixedness properties; we establish some of these properties in this section.

We use notation as in the introduction: we let $T$ and
$\bX=(X_1,\dots,X_n)$ be variables, let $\mV=(v_1,\dots,v_s)$ be
polynomials in $\KK[T,\bX]$, with $s \le n$, and let $\mU$ be a
polynomial matrix in $\KK[T,\bX]^{p \times q}$, with $p \le q$.
...
As in the previous section, we work with variables
$\bX=(X_1,\dots,X_n)$ and $T$.  We now consider polynomials
$\mG=(g_1,\dots,g_s)$ in $\KK[\bX]$, with $s \le n$, a polynomial
matrix $\mF$ in $\KK[\bX]^{p \times q}$, with $p \le q$ and we let
$\bC=(c_1,\dots,c_{s},\dots,c_m)$ be polynomials defined as follows:
$(c_1,\dots,c_{s})=(g_1,\dots,g_s)$, and $(c_{s+1},\dots,c_{m})$ are
the $p$-minors of $\mF$, so that $m=s+{q \choose p}$. The set that
interests us, $\VpFG{p}{\mF}{\mG}$, is the zero-set of~$\bC$. As in
the previous section, we assume $n=q-p+s+1$.

The goal in this section is to establish degree properties for certain
deformations of our system of equations $\bC$.  As in the previous
section, let $\mV=(v_1,\dots,v_s)$ be polynomials in $\KK[T,\bX]$,
with $s \le n$, and let $\mU$ be a polynomial matrix in $\KK[T,\bX]^{p
  \times q}$, with $p \le q$. Thus, $\mV$ and $\mU$ define a family of
polynomials, resp.\ polynomials matrices depending on $T$; for $\tau$
in $\KKbar$, we let $\mV_{T=\tau}$ and $\mU_{T=\tau}$ be respectively
the polynomials and polynomial matrix over $\KKbar[\bX]$ obtained
through evaluation of $T$ at $\tau$. We finally define polynomials
$\bB=(b_1,\dots,b_m)$ in $\KK[T,\bX]$ as before, by setting
$(b_1,\dots,b_s)=(v_1,\dots,v_s)$ and letting $(b_{s+1},\dots,b_m)$ be
the $p$-minors of $\mU$. As above, for $\tau$ in $\KKbar$, we write
$\bB_{T=\tau}=(b_{\tau,1},\dots,b_{\tau,m})=\bB(\tau,\bX)\subset
\KKbar[\bX]$.

Let us assume that $\mV_{T = 1} = \mG$ and $\mU_{T=1}=\mF$, that is,
the specialization of $\mU$ and $\mV$ at $T=1$ gives us $\mG$ and
$\mF$. This implies that $\bB_{T=1}=\bC$.

The main result in this section is the following; it relates the
number of isolated solutions, counted with multiplicities, of the
system $\bB$ at $T=1$, and more generally at any value $\tau \in
\KKbar$, to those for the same system at $T=0$, assuming the latter
...
\item[$\assG_1.$] For $k=1,\dots,m$,
  $\deg_\bX(b_k)=\deg_\bX(b_{0,k})$, where $\deg_\bX$ denotes the
  degree in $\bX$.
\item[$\assG_2.$] The only common solution to
  $b_{0,1}^H(0,\bX)=\cdots=b_{0,m}^H(0,\bX)=0$ is
  $(0,\dots,0)\in\KKbar{}^n$, where for $k=1,\dots,m$, $b_{0,k}^H$ is
  the polynomial in $\KKbar[X_0,\bX]$ obtained by homogenizing
  $b_{0,k}$ using a new variable $X_0$. 
\item[$\assG_3.$] The ideal $\langle \bB_{T=0} \rangle$ is radical in $\KKbar[\bX]$.
\end{description}
Then, $\bB_{T=0}$ has exactly $c$ solutions, all of them having multiplicity one.
\end{proposition}
...
neighborhood of it.  We already emphasized that the field $\SS$ is an
...
\begin{proof}
  ...
  that $(\psi_{i,1},\dots,\psi_{i,n})=(0,\dots,0)$, in view of $\assG_2$.
...
\begin{proof}
  ...
  $\langle \bB_{T=0} \rangle$ is radical (by assumption $\assG_3$) and
  zero-dimensional (by assumption $\assG_2$), the Jacobian matrix
  of $\bB_{T=0}(\bX)=\bB(0,\bX)$ has full rank $n$ at $\varphi_i$. Since
...
establish that $V(\bB_{T=0})$ consists of exactly $c$ solutions, all
with multiplicity one. Assumption $\assG_2$ implies that
$V(\bB_{T=0})$ is finite, and $\assG_3$ shows that the multiplicities
of all solutions is indeed one. We need to establish that the number of 
solutions at $T=0$ is indeed $c$.

Lemma~\ref{lemma:vPi} shows that $\bx$ is in $V(\bB_{T=0})$ if and
only if $(0,\bx)$ is in $V(J' + \langle T\rangle)$. Next, remark that
the two previous lemma taken together imply that $c=\kappa$; thus, in
view of Lemma~\ref{lemma:Z1}, to conclude, it is enough to prove that
for $i,i'$ in $\{1,\dots,c\}$, with $i \ne i'$, we have $\varphi_i \ne
\varphi_{i'}$.

Suppose to the contrary that $\varphi_i = \varphi_{i'}$. We know that
the Jacobian matrix of $\bB_{T=0}$ has full rank $n$ at $\varphi_i$; up to
...
\item a zero-dimensional parametrization $\scrR_0
  ...
  of $V(\bB_{T=0})$; the linear form $\lambda$ needs to satisfy some
...
$\bB_{T=0}$; this is the object of the following sections.
...
\textcolor{white}{{\bf Input}:} a zero-dimensional parametrization $\scrR_0$ of the system $\bB_{T=0}$\\
...
complete homotopy algorithm that takes into account  the column
degree structure of our matrices. As in the previous sections, we are
given the matrix $\mF =[f_{i,j}]\in \KK[X_1,\dots,X_n]^{p \times q}$
and polynomials $\mG=(g_1,\dots,g_s)$ in $\KK[X_1,\dots,X_n]$, with $p
\leq q$ and $n = q-p+s+1$; we show how to compute either the isolated points, or
the simple points, of
$$\VpFG{p}{\mF}{\mG} = \{\bx \in \KKbar{}^n \mid
\mathrm{rank}(\mF({\bx})) < p \text{~and~}
g_1(\bx)=\cdots=g_s(\bx)=0\}.$$ To match the notation of the previous
sections, we also let $\bC=(c_1,\dots,c_{s},\dots,c_m)$ be defined as
follows: $(c_1,\dots,c_{s})=(g_1,\dots,g_s)$, and
$(c_{s+1},\dots,c_{m})$ are the $p$-minors of $\mF$, so that $m=s+{q
  \choose p}$ and $\VpFG{p}{\mF}{\mG}$ is the zero-set of~$\bC$.
...
section.  These algorithms do not specify how to build the homotopy
deformation itself, or how to solve the start system of the homotopy. This is
...
$\mG$, the construction used in this section is already in the appendix
of~\cite{NieRan09}, where it was used to bound the number of solutions
of determinantal systems.
...
As in the previous sections, let then $T$ be the homotopy deformation
variable. We define the polynomials $\mV=(v_1,\dots,v_s)$ by 
$\bV = (1-T) \cdot \bM + T \cdot \mG$,  the matrix $\mU=(1-T)\cdot
  \bL + T \cdot \mF \in \KK[T,\bX]^{p\times q}$, and we let $\bB$ be the
polynomials in $\KK[T,\bX]$ given by $\bB=(b_1,\dots,b_s,\dots,b_m)$,
where
\begin{itemize}
\item $b_i=v_i$ for $i=1,\dots,s$
...
see that $\bB_{T=1}=\bC$. We also define
$\bA=(a_1,\dots,a_s,\dots,a_m)$ by $\bA=\bB_{T=0}$.  
...
\begin{example}\label{ex:12pts}
  ...
  only if of the conditions $\lambda_1(\bx)=\lambda_2(\bx)=0$,
...
a representation of the solutions of $\bA=\bB_{T=0}$ and an upper
...
\begin{example}
  ...
  subsitute $T=1$ in $\scrS$, since most denominators vanish. Instead,
...
In this section, the setting is slightly different from that seen up
...
variables $X_0,\dots,X_n$, and we prove several properties that will
be used in our row-degree homotopy algorithm. First, a notation: for
any ring $R$ and any matrix $\bM \in R^{p\times q}$, if $S$ is a
subsequence of $(1,\dots,p)$ and $U$ a subsequence of $(1,\dots,q)$,
$\bM_{S,U}$ is the submatrix of $\bM$ obtained by keeping rows indexed
...
in $\P^n(\KKbar)$ by the $p$-minors of $\bN^H$ and $\bM^H$ (note that
these minors are all homogeneous). In the rest of this section, if
$\bA^H$ is a matrix with polynomial entries that are homogeneous in
$X_0,\dots,X_n$, we use the notation $V_p(\bA^H)$ to denote the
...
this should cause no confusion. The first key result stated in
this section is the following; since the proof is rather lengthy
and technical, we postpone it to the appendix of the paper.
...
\begin{proposition}\label{lemma:appendix}
  ...
  \begin{itemize}
  \item the projective algebraic sets $V_p(\bM^H)$ and $V_p(\bN^H)$
    have no solution at infinity (that is, with $X_0=0$);
  \item the Jacobian matrices of $I_p(\bM^H)$ and $I_p(\bN^H)$ with
...
matrices without a homogeneous structure. Thus we will consider 
matrices $\bM$ and $\bN$ as in~\eqref{eqdef:type1}, but with $X_0=1$. Explicitly,
we have
\begin{align}\label{eqdef:type2aff}
  \bM= \left( \begin{matrix}
...
of the coefficients of these linear forms, both $V_p(\bM) \subset
\KKbar{}^n$ and $V_p(\bN) \subset \KKbar{}^n$ are a finite sets.

The second result in this section is an algorithm called
$\mathsf{RowDegreeDiagonal}$, that takes as input the linear forms
$(\lambda_{i,j,k})$ and computes a zero-dimensional parametrization of
the latter set $V_p(\bN)$. The key to do so is to observe that $\bx$
belongs to $V_p(\bN)$ if and only if some diagonal terms of $\bN$
vanish at $\bx$, say
$\lambda_{i_1,i_1}(\bx)=\cdots=\lambda_{i_\kappa,i_\kappa}(\bx)=0$ for
some indices $1 \le i_1 < \cdots < i_\kappa \le p$, all other
$\lambda_{i,i}(\bx)$ being non-zero, and if the submatrix of $\bN$
...
coefficients of the linear forms $\lambda_{i,j,k}$, to find all points
$\bx$ in $V_p(\bN)$, it is enough to do the following:
\begin{enumerate}
...
\item For any such $\bi,\br$, apply Gaussian elimination to the linear
  ...
  to rewrite $(X_{n-\kappa+1}, \ldots, X_n)$ as linear forms in $(X_1,
  \ldots, X_{n-\kappa})$.
\item If $\kappa = n$, we are done. Otherwise, use the above to
  eliminate $(X_{n-\kappa+1}, \ldots, X_n)$ in the submatrix
  $\bN_{\bi,(p+1, \ldots, q)}$ of $\bN$ of row indices $\bi$ and
...
coefficients of the linear forms $\lambda_{i,j,k}$, we never obtain 
twice the same point from for different choices of $\bi$ and $\br$.

\begin{example}\label{ex:N}
  In the next section, we will work with $\bN \in \K[X_1,X_2]^{2\times 3}$ given by
$$\bN = \left [\begin{matrix}
-X_1 + 10 X_2 + 1 & 0 & X_1+X_2 + 2\\
  ...
  \item With $\bi=(1)$, we can only take $\br = (1)$; the equation
    $-X_1 + 10 X_2 + 1=0$ gives the subsitution $X_2=(X_1-1)/10$,
    which we inject in $\bN_{(1),(3)} = [ X_1+X_2 + 2 ]$, to find the 
    solution $(-19/11, -3/11)$.
  \item With $\bi=(2)$, we first take $\br=(1)$, so we use
    $X_1-3X_2+5$ to obtain $X_2 = (X_1+5)/3$; we inject this expression into
    $\bN_{(1),(3)}=[(3X_1 +2 X_2-1)(2X_1 -3X_2 +5)]$ to find two solutions, namely
  ...
  \item With $\bi=(1,2)$, we can take $\br=(1,1)$ or $\br=(1,2)$,
    ...
    deal with the matrix $\bN_{\bi,(p+1, \ldots, q)}$.
  \end{itemize}
  Altogether, this gives us the 7 points where the rank of $\bN$ is
  not $2$.
\end{example}

In this example, matrix $\bN_{\bi,(p+1, \ldots, q)}$ has one column,
...
and a system of equations $\mG$, and computes the simple points of
...
We give an algorithm $\mathsf{RowDegree\_simple}$ in the next section;
...
matrix is as matrix $\bM$ in~\eqref{eqdef:type2aff}. Each entry of
such a matrix $\bM$ can be computed in $O((q-p) \alpha_i)$ operations
in $\KK$ (since we multiply $\alpha_i$ linear forms in $q-p+1$
variables), so that the whole matrix $\bM$ can be computed by a
...
\begin{align}
T_{\bM,{\rm row}}(\balpha,q)=T_{\rm row}((q-p)q(\alpha_1+\cdots+\alpha_p),(),\balpha,q).
\end{align}
...
the entries of 
$\bN \in \KK[X_1, \ldots, X_n]^{p \times q}$ as in~\eqref{eqdef:type1aff}, with $p \leq q$ and $n = q-p+1$\\
{\bf Output}: a zero-dimensional parametrization $\scrR$ of $V_p(\bN)$
\begin{enumerate}
...
\hfill $\text{\sf{cost:~}} O(\sum_{\bi,\br} n^3)$
    \item\label{step:constSLP} construct a straight-line program $\Gamma_{\bi,\br}$ that computes the matrix $\bM_{(\bi,\br)}$ in $\KK[X_1, \dots, X_{n-\kappa}]^{\kappa \times (n-1)}$ obtained
      by substituting $(f_{j,\bi,\br})_{n-\kappa+1 \le j \le n}$ into $\bN_{\bi,(p+1, \ldots, q)}$. The length of $\Gamma_{\bi,\br}$ is $O((n-\kappa) n(\alpha_{i_1}+\cdots+\alpha_{i_\kappa}))$.
...
\hfill $\text{\sf{cost:~}} \sum_{\bi,\br} T_{\bM,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), n-1)$
...
\begin{proof}
  ...
  of $\bN_{\bi,(p+1, \ldots, q)}$, in which we replace $X_j$ by
...
T_{\bM,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), n-1)\\
+\softO\left (\rc n^2  + \sum_{\substack{\bi=(i_1,\dots,i_\kappa)\\ \br=(r_1,\dots,r_\kappa)\\\kappa\le\min(n-1,p)}}
 ...
 \sum_{\substack{\bi=(i_1,\dots,i_\kappa)\\ \kappa\le\min(n-1,p)}}
  \alpha_{i_1}  \cdots \alpha_{i_\kappa}T_{\bM,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), n-1)\\
...
the construction in the previous section, we define $\bN$ as
...
(1-T) \cdot \bM + T \cdot \mG$ and the matrix $\mU=(1-T)\cdot \bL + T
\cdot \mF \in \KK[T,\bX]^{p\times q}$. As before, we let $\bB$ be the
polynomials in $\KK[T,\bX]$ given by $\bB=(b_1,\dots,b_s,\dots,b_m)$,
where
\begin{itemize}
\item $b_i=v_i$ for $i=1,\dots,s$
\item $(b_{s+1},\dots,b_{m})$ are the $p$-minors of $\mU$, taken in
  the same order as those in $\bC$.
\end{itemize}
Using the notation of Section~\ref{sec:homotopy}, we see that
$\bB_{T=1}=\bC$, and we define $\bA=(a_1,\dots,a_s,\dots,a_m)$ by
$\bA=\bB_{T=0}$. 
\begin{example}
  For $\mF$ as given in Example~\ref{ex:1}, we have $s=0$ and
  $(\alpha_1,\alpha_2)=(1,2)$. Thus, there are no polynomials $\bM$;
  we take $\bN$ as in Example~\ref{ex:N}.
\end{example}
Our next step is to prove that the assumptions of
Proposition~\ref{prop:degree_fiber} are satisfied for $\bB$, as long
as the coefficients of $m_1,\dots,m_s$ and $\bN$ are chosen
generically.
...
that the degree of all $p$-minors $(a_{s+1},\dots,a_m)$ of $\bN$ is
$\alpha_1 + \cdots + \alpha_p$.

Indeed, any $p$-minor of $\bN$ is of the form $\lambda_{i_1,i_1}
\cdots \lambda_{i_\kappa,i_\kappa} \zeta$, for some sequence
$\bi=(i_1,\dots,i_\kappa) \subset (1,\dots,p)$ of length $\kappa \in
\{0,\dots,p\}$ and some $(p-\kappa)$-minor $\zeta$ of $\bN_{\bi
,(p+1,\dots,q)}$.  Since the entries of $\bN_{\bi
...
$p$-minor of $\bN$ has degree $\alpha_1 + \cdots + \alpha_p$, as
claimed.
...
each $p$-minor of $\bN$ have degree $\alpha_1 + \cdots + \alpha_p$.)
...
$\bN^H$ can be rewritten as 
\begin{align*}
 \bN^H_\bu = \left( \begin{matrix}
...
Remark that the entries of $\bN^H_\bu$ are products of homogeneous
...
Proposition~\ref{lemma:appendix} implies that there is no projective
solution to $I_p(\bN^H_\bu)$ satisfying $X_0=0$. Taking into account
...
all $k$, and such that
and $\bN^H_\bu$ has rank less than $p$ at $\tilde\bx'=(1,x_1,\dots,x_{n-s})$.  The second item of
Proposition~\ref{lemma:appendix} shows that the Jacobian matrix of
$I_p(\bN^H_\bu)$ with respect to $X_0,\dots,X_{n-s}$ has rank $n-s$
at $\tilde \bx'$. Since the first coordinate of $\tilde\bx'$ is non-zero,
and all generating polynomials of $I_p(\bN^H_\bu)$ are homogeneous,
Euler's relation implies that the Jacobian matrix of $I_p(\bN_\bu)$
with respect to $X_1,\dots,X_{n-s}$
has full rank $n-s$ at $\bx'=(x_1,\dots,x_{n-s})$, where  
\begin{align}\label{eqdef:Lu}
 \bN_\bu = \left( \begin{matrix}
...
Next, we consider the $p$-minors $(a_{s+1},\dots,a_m)$ of $\bN$. Let
$\zeta \in\KK[X_1,\dots,X_n]$ be a $p$-minor of $\bN$, and let
$\zeta_\bu \in \KK[X_1,\dots,X_{n-s}]$ be the polynomial obtained
after applying the substitution in~\eqref{eq:subst} in $\bN$. Since
...
$p$-minor of $\bN_\bu$ as defined in~\eqref{eqdef:Lu}, and that all
...
\begin{lemma}
  ...
  \times q$ matrices $\bN$ as in~\eqref{eqdef:type1aff} or $\bM$ as
  in~\eqref{eqdef:type2aff}, with entries in $t=q-p+1$ variables,
$V_p(\bN)$ and $V_p(\bM)$ have
  cardinality $S_{t}(\alpha_1,\dots,\alpha_p)$.
\end{lemma}
\begin{proof}
  First, let us show that if the claim holds for $\bN$ in size $p
  \times q$, it holds for $\bM$ as well. To this effect, we set up a
  homotopy between $\bN$ and $\bM$, by considering the matrix
  $(1-T)\cdot\bN + T\cdot\bM$.  The discussion in the previous
  paragraphs shows that (for generic choices of the coefficients) this
  matrix satisfies Properties $\assA_1$ and $\assA_2$, together with
  $\assG_1,\assG_2,\assG_3$.  We claim that the same properties
  (degree bound, no solution at infinity, no multiplicities) hold as
  well at $T=1$: the degree property at $T=1$ is proved as we did for
  $T=0$ above, and the latter two are restatements of
  Proposition~\ref{lemma:appendix}. As a result, we can apply
  Proposition~\ref{prop:degree_fiber} to the specializations of
  $(1-T)\cdot\bN + T\cdot\bM$ at both $T=0$ and $T=1$, and conclude
  that $V_p(\bN)$ and $V_p(\bM)$ have the same cardinality, for
  generic choices of the coefficients of~$\bN$~and~$\bM$.

  We finish the proof by induction. If $p=q$, then $t=1$, $\bN$ is
  diagonal, and its determinant has degree $\alpha_1 + \cdots +
  \alpha_p = S_1(\alpha_1,\dots,\alpha_p)$, so our claim holds for
  $\bN$ (and thus for $\bM$). Suppose now that the claim is true for
  ...
  $$
  |V_p(\bN)| = \sum_{\substack{\bi=(i_1,\dots,i_\kappa)\\\br=(r_1,\dots,r_\kappa)}}  |V_\kappa(\bM_{\bi,\br})|,
  ...
  $\bM_{(\bi,\br)}$ is from Step~\ref{step:constSLP} of that algorithm.
  ...
  $$
  |V_p(\bN)| = \sum_{\substack{\bi=(i_1,\dots,i_\kappa),\\\br=(r_1,\dots,r_\kappa)}}  S_{t-\kappa}(\alpha_{i_1},\dots,\alpha_{i_\kappa}),
  $$
  ...
  $$
  |V_p(\bN)| = \sum_{\bi=(i_1,\dots,i_\kappa)} \alpha_{i_1}\cdots \alpha_{i_\kappa} S_{t-\kappa}(\alpha_{i_1},\dots,\alpha_{i_\kappa}),
  $$
...
\begin{proof}
  ...
  equal to the number of points in $V_p(\bN_\bu)$; this is a
...
We first show how to apply Proposition~\ref{prop:compute_regular} to
compute the {\em simple} points in $\VpFG{p}{\mF}{\G}$. Indeed, the
...
computes $\bB=(b_1,\dots,b_m)$ using $\sigma'=\sigma + \sigma_\bN +
O({q \choose p} n^3 + n^2 \gamma)=\sigma +O( {q \choose p} n^3 + n^2 p
\alpha+ n^2\gamma)$ operations.

\paragraph{A zero-dimensional parametrization $\scrR_0$ of $V(\bA)$.} 
To perform the homotopy, we need the solutions of the starting system,
...
computes the entries of matrix $\bN_\bu$ from~\eqref{eqdef:Lu}: it
...
zero-dimensional parametrization $\scrR'_\bu$ of $V_p(\bN_\bu)$.  The
...
\alpha_{i_1} \cdots \alpha_{i_\kappa} T_{\bM,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), n-s-1)
+
...
\item construct a straight-line program $\Delta$ that computes
  $\bN \in \KK[X_1,\dots,X_n]^{p \times q}$ as
...
\item construct a straight-line program $\Delta_{\bu}$ that computes the matrix 
  $\bN_\bu  \in \KK[X_1, \dots, X_{n-s}]^{p \times q}$ obtained
  by substituting $(\phi_{k,\bu})_{n-s+1 \le k \le n}$ into $\bN$
...
last step. To estimate its complexity, we apply
...
Algorithm {\sf RowDegree\_simple}.  Now, we write $\sigma'+ n^2 =
\sigma +O( {q \choose p} n^3 + n^2 p \alpha + n^2 \gamma)$, for which
we use the upper bound ${q \choose p} n^3(\sigma + p\alpha + \gamma)$
(recall $\alpha=\max(\alpha_1, \ldots, \alpha_p)$ and
...
This will now allow us to give an estimate on $T_{\bM,{\rm row}}$ by
solving a few recurrence relations. Recall that $T_{\bM,{\rm row}}$ describes
the case where $s=0$, so that $\gamma_1 \cdots \gamma_s=1$, and $\bM$
...
then written $T_{\bM,{\rm row}}((\alpha_1,\dots,\alpha_p),q)$.

\begin{lemma}\label{lemma:TMrow}
  One can take
  $$T_{\bM,{\rm row}}((\alpha_1,\dots,\alpha_p),q)= \softO\left ({q
...
\begin{proof}
  ...
  recursion
  \begin{multline}
  T_{\bM,{\rm row}}((\alpha_1,\dots,\alpha_p),q)=
\sum_{\substack{\bi=(i_1,\dots,i_\kappa)\\ \kappa\le\min(q-p,p)}}
\alpha_{i_1} \cdots \alpha_{i_\kappa} T_{\bM,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), q-p)
\\ + \softO\Big(
...
This term is at its maximum at
  ...
  $T_{\bM,{\rm row}}$ by finding a solution to the recurrence
...
$$T_{\bM,{\rm row}} \le   (q-p+1) S_{q-p+1} 
...
we obtain that a term such as $T_{\bM,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}),q-p)$
is
$$
...
% A homotopy matrix is $\mU = (1-T)\cdot \bM_C + T\cdot C$. After the
...
these minors are all homogeneous). In the rest of this section, if
$\bA^H$ is a matrix with polynomial entries that are homogeneous in
$X_0,\dots,X_n$, we use the notation $V_t(\bA^H)$ to denote the
...
\begin{proposition}\label{lemma:appendix2}
  For generic choices of the coefficients of the linear forms
  $\lambda^H_{i,j,k}$, the following holds:
  \begin{itemize}
  \item the projective algebraic sets $V_p(\bM^H)$ and $V_p(\bN^H)$
...
$\bM \in R^{p\times q}$, if $S$ is a subsequence of $(1,\dots,p)$ and
$T$ a subsequence of $(1,\dots,q)$, $\bM_{S,T}$ is the submatrix of
$\bM$ obtained by keeping rows indexed by $S$ and columns indexed by
$T$. We also call this the $(S,T)$-submatrix of $\bM$.
...
this matrix is thus the ``generic'' model of the matrix $\bM^H$ seen previously.
