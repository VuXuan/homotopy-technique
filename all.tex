...
In several of these situations, the system which consists of
$G, M_p(\mF)$ (recall that we assume $p \leq q$) is radical and
zero-dimensional ; see e.g. \cite{BGHSS}. In that case, the Jacobian
matrix associated to $G, M_p(F)$ has maximal rank at any point of
$\VpFG{p}{\mF}{G}$. Hence, it also makes sense to look at the
following slight variant of Problem~\ref{problem2}.
...
\begin{pbm} \label{problem3} Given a field $\KK$, a matrix
  ...
  $n = q-p+s+1$, compute the subset of $\VpFG{p}{\mF}{G}$ at
  which the Jacobian Matrix of $G, M_p(F)$ has maximal rank.
\end{pbm}
...
$E_k(A_1,\dots,A_q)$ is the elementary symmetric polynomial
of degree $k$ in $(A_1, \ldots, A_q)$ and
$S_k(B_1,\dots,B_p)$ is the $k$th complete symmetric
polynomial in $(B_1,\dots,B_p)$.
...
The complexity of solving determinantal polynomial systems has also
been studied through Gr\"obner bases. Starting from the determination
of the Hilbert function of a determinantal ring due to Conca and
...
$M_r(\mF)$ when $r\leq p\leq q$ or $G, M_{p}(\mF)$ where $\mF$ is the
Jacobian matrix associated to $G$ with one extra row) but under some
genericity assumptions on the entries of $\mF$ or $G$. Besides, they
are assumed to all have the same degree. This latter assumption is
removed by Spaenlehauer in \cite{Spa14} where this series of works
culminate with sharp complexity statements but still under genericity
assumptions.
...
generalization in \cite{SaSp16} assuming that the dimension of the
degeneracy locus is as expected. As our algorithm here, these
...
varieties defined by the considered intermediate systems and, even
without taking into account the dimension assumption, it is not clear
how this compares to the above results.
...
\end{proposition}
The next proposition will be used further when solving Problem~\ref{problem3}.
\begin{proposition}\label{prop:compute_regular}
  Under the previous assumptions and letting $c$ be as in
  Proposition~\ref{prop:degree_fiber}, we can compute a
  zero-dimensional parametrization of the isolated points of $V(\bC)$
  using
$$\softO() \subset (e\,\sigma\,m)^{O(1)}$$ operations in~$\KK$.  
\end{proposition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Proof of Propositions~\ref{prop:compute_isolated} and~\ref{prop:compute_regular}} \label{proof:prop2}
...
$V(J' + \langle T-1\rangle) =\{\varphi'_i \mid i=1,\dots,{\kappa'}\}$
starting from $\scrS$, in time $\softO(c\,e\,n)$.
