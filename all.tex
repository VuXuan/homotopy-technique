...
$$\softO\left (c(e+c^5 )(\sigma + {q \choose p}n^3+n^2\gamma + nq
  \delta)\right),$$ since one easily checks that $e \ge 2^n$,
so that $n \in \softO(e)$. A
...
\[
  \softO\left ({q \choose p}  c(e+c^5 )n^3(\sigma +\gamma + q
    \delta)\right).
\]
Using again  that $n\le\log(e)$, we can omit the factor
...
All cost  estimates were given in the previous subsection
and are summarized in~\eqref{eq:VbA}, save for that of 
the last step. To estimate its complexity, we apply
...
Algorithm {\sf RowDegree\_simple}.
Now, we 
write $\sigma'+ n^2 = \sigma +O( {q \choose p} n^3 + n^2 p \alpha + n^2 \gamma)$,
for which we use the upper bound ${q \choose p} n^3(\sigma + p\alpha + \gamma)$.
...
\end{align*}
with $T$ as in~\eqref{eqdef:T}. Since $\re \ge 2^n$, this becomes
