...
\begin{theorem}
  \todo{$G$ should be integrated ; we also need to define $\delta$.}
...
\item return $\mathsf{Homotopy}(\Gamma',\scrR)$ ($\Gamma'$ as in the previous paragraph)\\
...
$\mathsf{RowDegree}(\mF,G)$ which takes as input a polynomial matrix
$\mF$ a system of equations $G$, and solve Problem~\ref{problem2} for
this input using a row-degree homotopy. We give such an algorithm in
the next section. In our case, we do not need the extra set of
equations $G$. 

The cost reported at each step is the total amount of time spent
there, over all iterations. We denote by $T_{\rm
  row}(\bgamma,\balpha,q)$ the time spent by $\mathsf{RowDegree}(\mF,G)$
on input $\mF$ with row degrees $\balpha=(\alpha_1,\dots,\alpha_p)$
and $q$ columns, and $G$ of degrees $\gamma=(\gamma_1,\dots,\gamma_s)$.
...
\item for any subsequence $\bi = (i_1, \ldots, i_\kappa)$ of $(1, \ldots, p)$ with $1 \leq \kappa \leq\min(n-1,p)$
  ...
  \item for any sequence $\br = (r_1, \ldots, r_\kappa)$, with $r_k \in \{1,\dots,\alpha_k\}$ for all $k$
    ...
    \item apply Gaussian elimination to the system $\lambda_{i_1,i_1,r_1}=\dots=\lambda_{i_\kappa,i_\kappa,r_\kappa}=0$
      to rewrite $(X_{n-\kappa+1}, \ldots, X_n)$ as linear forms in $(X_1, \ldots, X_{n-\kappa})$. \\
$\text{\sf{cost:~}} O(n^3 S_n(\alpha_1,\dots,\alpha_p))$
      
    \item substitute $(X_{n-\kappa+1}, \ldots, X_n)$ into $\mN_{\bi,(p+1, \ldots, q)}$ to obtain a matrix $\mM_{\bi,\br} \in \KK[X_1, \dots, X_{n-\kappa}]^{\kappa \times (n-1)}$\\
$\text{\sf{cost:~}} \softO(\cdots)$
    \item $\scrR_{\bi,\br}' \gets$ $\mathsf{RowDegree}(M_{\bi,\br},\{\})$  (points have coordinates $(X_1, \ldots, X_{n-\kappa})$)\\
$\text{\sf{cost:~}} \softO(\cdots)$
    \item deduce $\scrR_{\bi,\br}$ from $\scrR_{\bi,\br}'$ by adding the expressions for $(X_{n-\kappa+1}, \ldots, X_n)$\\
$\text{\sf{cost:~}} \softO(\cdots)$
  \end{enumerate}
  \end{enumerate}
...
The construction of the matrix $\mL$ presented in Section~\ref{sec:columndegree}
...
construction in the previous section, we define
...
degree of all $p$-minors $(a_{s+1},\dots,a_m)$ of $\mL$ is $\alpha_1 +
\cdots + \alpha_p$.
...
\begin{proof}
  ...
  $t=n-s=q-p+1$ variables and with row degrees $\alpha_1,\dots,\alpha_p$.
  We prove that for any matrix $\mN$ as in~\eqref{eqdef:type1aff},
  and any matrix $\mM$ of 
  the form
  \begin{align}\label{eqdef:type2aff}
    ...
    \end{matrix} \right),
  \end{align}
  where $\lambda_{i,j}$ is a product of $\alpha_i$ (non necessarily
  homogeneous) linear forms
  $\lambda_{i,j,1},\dots,\lambda_{i,j,\alpha_i}$ in $t$ variables, if
  the coefficients of these linear forms are generic, $V_p(\mN)$ and
  $V_p(\mM)$ have cardinality $S_{t}(\alpha_1,\dots,\alpha_p)$.
  ...
  for each subsequence $(i_1,\dots,i_n)$ of $(1,\dots,p)$. Altogether, 
  we obtain 
  $$
...
Since $\assG_1(0),\assG_2(0),\assG_3(0)$ hold, we are going to apply
Proposition~\ref{prop:compute_isolated} to compute the isolated points
in $S_{\mF,G}$. In the following paragraphs, we discuss properties
$\assD_1,\assD_2,\assD_3$ in this context.

\paragraph{Property $\assD_1$.} To obtain a zero-dimensional parametrization of
$V(\bA)$, we use Algorithm {\sf RowDegreeDiagonal} given in the 
previous section.

%% .  We do this by following the description of the solutions of
%% $\bA$ given in the previous paragraph: for any choice of indices
%% $\bu$, $\bj$ and $\bv$ as above, the corresponding point $\bx \in
%% \KKbar{}^n$ in $V(\bA)$ can be found by solving a linear system of
%% size $n$, so in time $O(n^3)$. We repeat this procedure
...
\begin{algorithm}
\caption{$\mathsf{RowDegree}$}
{\bf Input}:  matrix $F \in \KK[X_1, \ldots, X_n]^{p \times q}$ with $\deg(f_{i,j}) \leq \delta_i$\\
\textcolor{white}{{\bf Input}:} polynomials $G = (g_1, \ldots, g_s)$ in $\KK[X_1, \ldots, X_n]$ with $p \leq q$, $n = q-p+s+1$\\
{\bf Output}: a zero-dimensional parametrization of the isolated points of $S_{\mF,G}$
\begin{enumerate}
\item construct 
 matrix $\mL \in \KK[\bX]^{p \times q}$ as in~\eqref{eqdef:startrow}
and $(a_1, \ldots, a_s)$ in $\KK[\bX]$ as in~\eqref{eqdef:ai}
\item for any sequence $\bu=(u_1,\dots,u_s)$, with $u_j \in \{1,\dots,\gamma_j\}$ for all $j$
\begin{enumerate}
...
to rewrite $(X_{n-s+1}, \ldots, X_n)$ as linear forms in $(X_1,\dots,X_{n-s})$
\item substitute $(X_i)_{n-s+1 \leq i \leq n}$ into $\mL$ to obtain $\mL_\bu\in \KK[X_1, \ldots, X_{n-s}]^{p \times q}$
\item $\scrR'_\bu \gets \mathsf{RowDegreeDiagonal}(\mL_\bu)$ (points have coordinates $(X_1,\dots,X_{n-s})$
\item deduce $\scrR_\bu$ from $\scrR'_\bu$ by adding the expressions for $(X_{n-s+1},\dots,X_n)$ 
\end{enumerate}
\item combine all $\scrR_\bu$ into $\scrR$
\item return $\mathsf{Homotopy}(\bB,\scrR)$ ($\bB$ as in the previous paragraphs)
\end{enumerate}
\label{Row}
\end{algorithm}
