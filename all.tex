...
\begin{proposition}\label{prop:degree_fiber}
  {\bf xxx -- The proof uses the Jacobian criterion, so there must be
    some assumptions on the characteristic of $\KK$.} Suppose that
  assumptions $\assA_1$ and $\assA_2$ hold. Then, there exists an
...
\begin{proposition}\label{prop:compute_regular}
  ...
  Assume that $\KK$ has characteristic greater than {\bf xxx} and that
  ${\assD}_1, {\assD}_2$ and ${\assD}_3$ hold.

  Reusing the notations of Propositon~\ref{prop:compute_isolated}, we
  can compute a zero-dimensional parametrization of the simple
...
\begin{proof}
  ...
  criterion~\cite[Theorem~16.19.b]{Eisenbud95} {\bf xxx -- Here there
    should be some restriction on the characteristic of the field},
  this is the case if and only if the Jacobian matrix of $\bB$ with
  respect to $\bX$ has full rank $n$ at $\Phi_i$. We know that
  $\varphi_i=\lim_0(\Phi_i)$ is a root of $\bB_0$
  (Lemma~\ref{lemma:Z1}), and the Jacobian criterion conversely
  implies that since the ideal $\langle \bB_0 \rangle$ is radical (by
  assumption $\assG_3(0)$) and zero-dimensional (by assumption
  $\assG_2(0)$), the Jacobian matrix of $\bB_0(\bX)=\bB(0,\bX)$ has
  full rank $n$ at $\varphi_i$. Since this matrix is the limit at zero
  of the Jacobian matrix of $\bB$ with respect to $\bX$, taken at
  $\Phi_i$, the latter must have full rank $n$, and our claim that
  $\frak{J}'$ is radical is proved.
\end{proof}
...
{\bf xxx -- the existence of a well-separating element $\lambda$
  depends on the characteristic of $\KK$. If it is too small, such an
...
\begin{proposition}\label{prop:coldeg}
  ...
  When $\KK$ has characteristic $0$ or larger than ${\bf xxx}$, there
