...
$\mU=(1-T)\cdot \mL + T \cdot \mF$ and $\mL$. In view of the shape of $\mL$,
...
\gamma_i)$ operations for $(a_1,\dots,a_s)$, and $O(\Lambda+n \sum_i
...
$O(n^2 + \Lambda + n \sum_j \delta_j)$ operations. From that, all $p$-minors
...
$\Lambda'=O(\Lambda + {q \choose p} n^3 +n \sum_i \gamma_i + n \sum_j \delta_j)$.
...
$\softO\left (c(e+c^5 )(\Lambda + cn^4)\right).$
...
$\mathsf{cost: \softO\left (c(e+c^5 )(\Lambda + cn^4)\right)}$
\end{enumerate}
\label{ColHom}
\end{algorithm}
