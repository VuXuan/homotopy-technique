...
Our second result assumes that $\KK$ has characteristic $0$ and then
gives bounds on the cost of computing a zero-dimensional
parametrization of the isolated solutions of
...
\begin{theorem}\label{theo:2}
  Assume that $\KK$ has characteristic $0$. Suppose that matrix
  $\mF \in \KK[X_1,\dots,X_n]^{p \times q}$ and polynomials
...
complexity estimates. {\bf Below, we let $\chi = \max()$ xxx;
  $\chi$ will be used for the minimum possible characteristic,
  i.e. the max of the degrees and stg like the square of the
  max. number of solutions. }

\begin{theorem}\label{theo:3}
  Suppose that $\KK$ has characteristic greater than $\chi$ and
  that the matrix $\mF \in \KK[X_1,\dots,X_n]^{p \times q}$ and
...
detail.  As a preliminary, when $\KK$ has characteristic $0$, we will
need an algorithm which takes as input polynomials
$\bC=(c_1,\dots,c_m)$ and a point $\bx$ in the zero-set of $\bC$, and
which decides whether $\bx$ is an isolated points of $V(\bC)$. This
one will be used to solve Problem~\eqref{problem2}.
...
In this whole section, we assume that $\KK$ has characteristic
$0$. Let $\LL$ be a field containing the field $\KK$ and $\LLbar$ be
an algebraic closure of $\LL$.  Let $\bC=(c_1,\dots,c_m)$ be
polynomials in $\KK[\bX]$, with $\bX=(X_1,\dots,X_n)$. Given a point
$\bx$ with coordinates in $\LL$ in the zero-set
$V(\bC)\subset \LLbar{}^n$, we discuss here how to decide whether
$\bx$ is an isolated point in $V(\bC)$. We make the following
...
\begin{proposition}\label{prop:compute_isolated}
  ...
  Assume that $\KK$ has characteristic zero and that
  ${\assD}_1, {\assD}_2$ and ${\assD}_3$ hold. Letting $c$ be as in
  Proposition~\ref{prop:degree_fiber}. There exists a probabilistic
  algorithm which computes a zero-dimensional parametrization of the
  isolated points of $V(\bC)$ using
  $$\softO(c^5 m n^2  + c(e+c^5 n)(\sigma + n^3)) \subset (e\,\sigma\,m)^{O(1)}$$
  operations in~$\KK$. 
\end{proposition}
...
\begin{proposition}\label{prop:compute_regular}
  ...
  Assume that $\KK$ has either characteristic $0$ or characteristic
  greater than $8\ n \ c^2$ and that ${\assD}_1, {\assD}_2$ and
  ${\assD}_3$ hold.
  ...
  operations in~$\KK$ with probability at least $7 / 8$.
\end{proposition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
To achieve this, we need to first introduce the field
$\SS = \KKbar(( T^\Q ))$ of generalized power series ring (also known
as Hahn series ? {\bf xxx}): all elements of $\SS$ can be written as
$f = \sum_{i\in \Q} c_i T^i$ with $c_i \in \KKbar$ and the set
$\iota(f) = \{i \mid c_i \neq 0\}$ is well-ordered (this set is called
exponent support set of $f$). In contrast to Puiseux series,
$\sum_{k\in \N-\{0\}} T^{-1/p^k}$ is a generalized power series while
it is not a Puiseux series over $\KKbar$ (because the denominators are
not bounded). The interest of considering $\SS$ is only for dealing
with situations where $\KK$ has positive characteristic (else, one can
work with Puiseux series which constitute a perfect field). Besides,
since $\KKbar$ is algebraically closed, $\SS$ is algebraically closed
too \cite[Theorem 1]{Mclane39} {\bf xxx check}. Actually, it is an
algebraic closure of $\KKbar(T)$ and hence is a perfect field too.

Also, as for Puiseux series, one can define naturally a valuation over
$\SS$ since the sets of exponent supports of its elements are
well-ordered ones. We give more details on this further.
...
\begin{proof}
  ...
  in $\LL' \rangle\rangle[T_0]$ where $\LL'$ is the generalized Power
...
\begin{proof}
  ...
  $\mathfrak{m}_{\Phi_i}$ is a field.  Observe that
  $\KKbar\langle\langle T \rangle\rangle$ is a perfect field. By the Jacobian
...
satisfied for a generic choice of $\lambda$ when $\KK$ has
characteristic $0$, or with probability at least $7/8$ when $\KK$ has
characteristic greater than $n \ c^2$. When this is the case,
...
$\mathsf{Homotopy\_regular}$. It differs from algorithm
...
\begin{proposition}\label{prop:coldeg}
  ...
  When $\KK$ has characteristic $0$, there exists a
  randomized algorithm that computes these isolated points
  $$\softO\left (c(e+c^5 )(\sigma + c)\right)$$
  operations in $\KK$.

  When $\KK$ has characteristic $0$ or larger than $8 \ n c^2$, there
...
cost simplifies to
$$\softO\left ( 
  \left (n+{q \choose p}\right )n^2\, c^2\, +\, n\left (\left (n+{q
        \choose p}\right ) \sigma + {q \choose p} n^3
    (s\,\gamma+q\,\delta)\right ) c\, e \right ).$$ This concludes the
proof of Proposition~\ref{prop:coldeg}. The resulting algorithm,
called $\mathsf{ColumnDegree\_regular}$ differs from
$\mathsf{ColumnDegree\_regular}$ only at the last step where Algorithm
$\mathsf{Homotopy\_regular}$ is called instead.
