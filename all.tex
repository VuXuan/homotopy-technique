...
g_1,\dots,g_s\rangle$.  Our main problem is the following.
...
CellDecompSurface,BertiniReal,RealNumerical,
SaSc03,SaSc11,SaSc17},
...
says that $\bx$ isolated in $V(I)$ when there exists a unique primary
...
\begin{theorem}\label{theo:2}
  ...
  Problem~\ref{problem2} in either
   $$\softO\left (
...
%     c(e+c^5 )(\sigma + c)
  ...
  or 
  $$\softO\left(
    \rc^2 n^5 {q \choose p}^2 (\re+\rc^5 n)(\sigma + \chi')  
  \right)$$
  operations in $\KK$, with 
\begin{align*}
  c'&=\deg(g_1)\cdots\deg(g_s) S_{n-s}(\rdeg(\mF,1), \ldots, \rdeg(\mF,p))\\
...
\end{align*}
\end{theorem}
The assumption that all degrees be at least $1$ is not a restriction.
...
\begin{theorem}\label{theo:3}
  ...
  Then, there exists a probabilistic algorithm that solves
  Problem~\ref{problem3} in either
  ...
  \end{align*}
or 
$$\softO\left( \rc^2 n^6{q \choose p}^2 (\rc + \re ( \sigma + \chi' ) ) \right)$$
operations in $\KK$, with 
\begin{align*}
  c'&=\deg(g_1)\cdots\deg(g_s) S_{n-s}(\rdeg(\mF,1), \ldots, \rdeg(\mF,p))\\
  e'&=(\deg(g_1)+1)\cdots(\deg(g_s)+1)S_{n-s}(\rdeg(\mF,1)+1, \ldots, \rdeg(\mF,p)+1) \\
...
\begin{proposition}\label{prop:compute_regular}
  ...
  of~$V(\bC)$ using
  $$\softO(m \,n^2 c^2 + n\, (m \sigma+n^2)\,c\,e )\subset (e\,\sigma\,m)^{O(1)}$$
  operations in~$\KK$. 
\end{proposition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
form. This form is used in the returned rational parametrization and
must satisfy some properties which are described further (see
Subsection~\ref{proof:prop2}). When the chosen form does not fulfill
these requirements, either the algorithm fails or the output
parametrization encodes a subset of the simple isolated $V(\bC)$.   
\end{remark}
...
belong to a positive-dimensional component of $V(\bC)$. Hence, when
$\KK$ has characteristic $0$, we can use the algorithm of
Section~\ref{sec:isolated}. By Proposition~\ref{prop:degree_fiber}, we
can take $c$ as an upper bound on the multiplicity of isolated
solutions of $\bC$.
...
. The resulting algorithm, which we simply name
$\mathsf{Homotopy}$, is described hereafter.
...
previous section to Problems~\ref{problem2} (resp.~\ref{problem3}), by
...
algorithm, called $\mathsf{ColumnDegree\_simple}$ differs from
...
$\KK$. Using similar simplifications as above, we obtain as a bound
\[
  \softO\left (
...
Problem~\ref{problem3} for this input using a row-degree homotopy. We
...
We now give algorithms to solve Problems~\ref{problem2}
and~\ref{problem3} whose runtime will depend on the row-degrees of the
...
designing an algorithm for computing the isolated points of
$\VpFG{p}{\mF}{G}$ whose cost depends on the row degrees
...
The homotopy on which this algorithm relies makes use of Algorithm
$\mathsf{RowDegreeDiagonal}$ given in the previous Section. This one
itsself uses an algorithm called $\mathsf{RowDegree\_simple}$ which,
on input $\mF$ and $G$, computes the simple isolated points of
$\VpFG{p}{\mF}{G}$.

\begin{proposition}\label{prop:rowdegree_simple}
  Reusing the notations introduced above and under the assumptions of
  Proposition~\ref{prop:rowdegree}, there exists a randomized
  algorithm that computes the simple isolated points of
  $\VpFG{p}{\mF}{G}$ using
  $$\softO\left( \rc^2 n^6{q \choose p}^2 (\rc + \re ( \sigma + nq p
    \alpha +\gamma +n^3 ) ) \right)$$ operations in $\KK$.
\end{proposition}

These propositions complete the proof of Theorems~\ref{theo:1},
\ref{theo:2} and~\ref{theo:3}. We start by solving
Problem~\ref{problem3} and hence give the algorithm
...
To solve Problems~\ref{problem2} and~\ref{problem3}, we are again
going to rely on the algorithm of Section~\ref{sec:homotopy}. As in
Section~\ref{sec:columndegree}, we let
$\bC=(c_1,\dots,c_s,c_{s+1}\dots,c_m)$ be such that
...
Proposition~\ref{prop:compute_isolated} In the following paragraphs,
we discuss the required properties $\assD_1,\assD_2,\assD_3$ in this
context. In what follows, we assume that we are given a straight-line
program $\Gamma$ of length $\sigma$ that computes the input matrix
$\mF$ and the input equations $G$.  Besides, we also assume that all
$\gamma_i$'s and $\alpha_j$'s are at least equal to $1$,
