...
\footnotetext[2]{Sorbonne Universit\'es, UPMC Univ. Paris 06, CNRS, INRIA Paris Center, LIP6, PolSys Team, France}
...
geometry~\cite{ARS,BaGiHeMb01,BaGiHePa05,BGHSS,BRSS,RealDecompICMS,
  CellDecompSurface,BertiniReal,RealNumerical, SaSc03,SaSc11,SaSc17},
where $\mF$ consists of the Jacobian matrix of $G$, together with one
extra row, corresponding to the gradient of a function that we want to
optimize on $V(G)$. Because they show up several times in this introduction, we will refer to this particular class of inputs 
as systems {\em coming from optimization}.
...
\begin{theorem}\label{theo:2}
  ...
  Then, there exists a probabilistic algorithm that solves
  Problem~\eqref{problem2} in either
   $$\softO\left (
     {q \choose p} n^3 c(e+c^5 )(\sigma +\chi)
%     c(e+c^5 )(\sigma + c)
   \right)$$
  ...
  \begin{align*}
    ...
    \chi&= q\max(\cdeg(\mF,i), 1\leq i \leq q)+ \max(\deg(g_i), 1\leq i \leq s)
  \end{align*}
  or 
  $$\softO\left(
   {q \choose p}^2  \rc^2 n^5 (\re+\rc^5 n)(\sigma + \chi')  
  \right)$$
  operations in $\KK$, with 
\begin{align*}
  c'&=\deg(g_1)\cdots\deg(g_s) S_{n-s}(\rdeg(\mF,1), \ldots, \rdeg(\mF,p))\\
  e'&=(\deg(g_1)+1)\cdots(\deg(g_s)+1)S_{n-s}(\rdeg(\mF,1)+1, \ldots, \rdeg(\mF,p)+1)\\
  \chi'&={q \choose p} n +\max(\rdeg(\mF, i), \leq i \leq p)+ \max(\deg(g_i),1\leq i \leq s)+ n^3.
\end{align*}
\end{theorem}
...
\begin{theorem}\label{theo:3}
  ...
  $$\softO\left ( {q \choose p}^2\,n^3\,c (c + e (\sigma+\chi))\right)$$
  operations in $\KK$, with
  \begin{align*}
  c&=\deg(g_1)\cdots\deg(g_s)E_{n-s}(\cdeg(\mF,1), \ldots, \cdeg(\mF,q))\\
    e&=(\deg(g_1)+1)\cdots(\deg(g_s)+1) E_{n-s}(\cdeg(\mF,1)+1, \ldots, \cdeg(\mF,q)+1) \\
  ...
  \end{align*}
or 
$$\softO\left( {q \choose p}^2 \rc^2 n^6 (\rc + \re ( \sigma + \chi' ) ) \right)$$
operations in $\KK$, with 
\begin{align*}
  c'&=\deg(g_1)\cdots\deg(g_s) S_{n-s}(\rdeg(\mF,1), \ldots, \rdeg(\mF,p))\\
  e'&=(\deg(g_1)+1)\cdots(\deg(g_s)+1)S_{n-s}(\rdeg(\mF,1)+1, \ldots, \rdeg(\mF,p)+1) \\
  \chi' &= n\ q\ p \max(\rdeg(\mF, i), 1\leq i \leq p)+ \max(\deg(g_i),1\leq i \leq s) + n^3.
\end{align*}  
\end{theorem}
...
Our algorithms are based on a {\em symbolic homotopy continuation}.
...
with an emphasis on the algebraic geometry underlying these
techniques. By contrast, their usage in symbolic contexts is more
...
unknowns; extensions of these methods can 
deal with systems of positive dimension by
essentially using variants of the algorithm for square systems.
...
Herzog~\cite{CH94}, complexity estimates are given in \cite{FSS13,
  FSS12} for computing Gr\"obner basis of ideals generated by either
$M_r(\mF)$ when $r\leq p\leq q$, or $G, M_{p}(\mF)$ (for inputs coming
from optimization problems), but under some genericity assumptions on
the entries of $\mF$ or $G$, which are also assumed to all have the
...
\begin{proposition}\label{prop:compute_isolated}
  ...
  probabilistic algorithm which computes a zero-dimensional
...
\begin{proposition}\label{prop:compute_regular}
  ...
  there exists a probabilistic algorithm which computes a
  zero-dimensional parametrization of the simple  points 
  of~$V(\bC)$ using
  $$\softO( c^2\, m \,n^2 + \,c\,e (m \sigma+n^2)\,n )\subset (e\,\sigma\,m)^{O(1)}$$
  operations in~$\KK$.
\end{proposition}
...
$$\softO(n^4 c^6 + n^2 m c^5 + n \sigma c^6)$$ operations in~$\KK$. Adding all
...
\item\label{step:homot:final} remove from $Z(\scrR_1)$ points that are not isolated in $V(\bC)$ \\
  $\text{\sf{cost:~}} \softO(n^4 c^6 + n^2 m c^5 + n \sigma c^6)$
\end{enumerate}
\label{DetSys}
\end{algorithm}
...
$w_1$, as done
previously when we were decomposing $\scrR_0$, using $\softO(\sigma' c)$ 
operations in $\KK$, \todo{from here} and use Gaussian
elimination to identify divisors of $w_1$ that need to be
removed. This Gaussian elimnation takes $\softO((mn^2+n\sigma')c)$
operations in $\KK$ which simplifies to $\softO(nm(n+ \sigma)c)$. The
final cleaning step is done using Algorithm $\mathsf{Clean}$ of
...
$$
which ends to prove Proposition~\ref{prop:compute_regular}. In the
...
The following sections will show how to apply the algorithm of the
...
isolated points) of $\VpFG{p}{\mF}{G}$, with
...
\begin{proposition}\label{prop:coldeg}
  ...
  points
  $$
  \softO\left ({q \choose p} n^3 c(e+c^5 )(\sigma +n^2\gamma + q
    \delta)\right)
  % c(e+c^5 )(\sigma + c)
  $$
  operations in $\KK$.
...
computes isolated simple points of $\VpFG{p}{\mF}{G}$.
\begin{proposition}\label{prop:coldeg_simple}
  Reusing the notations introduced above, and letting
  $\gamma=\max(\gamma_1, \ldots, \gamma_s)$ and 
  $\delta=\max(\delta_1, \ldots, \delta_q)$, 
  there exists a randomized algorithm that computes the simple
  isolated points of $\VpFG{p}{\mF}{G}$ using
  $$ 
  \softO\left (   {q \choose p}^2\,n^3\,c (c + e (\sigma+\gamma+q\delta)) \right )
  $$ 
  operations in $\KK$, 
\end{proposition}
...
$\softO\left (c(e+c^5 n)(\sigma + {q \choose p}n^3+n^2\gamma + q
  \delta)\right).$ This can also be rewritten as
$\softO\left (c(e+c^5 )(\sigma + {q \choose p}n^3+n^2\gamma + q
  \delta)\right)$, by means of a case discussion between $e$ and
$c^5 n$. If $c^5 n \le e$, then $e+c^5 n$ is in $O(e)$. Else, we have
$e \le c^5 n$; since $e \ge 2^n$, this implies that $c \ge 2^n/n$, so
that $n$ is in $O(\log(c))$, and $e+c^5 n$ is in $\softO(c^5)$. At
last, further simplification shows that the obtained bound can be
simplified to
\[
  \softO\left ({q \choose p} n^3 c(e+c^5 )(\sigma +n^2\gamma + q
    \delta)\right)
\]
which ends the proof of Proposition~\ref{prop:coldeg}.
...
$\mathsf{ColumnDegree\_simple}$ only at the last step where Algorithm
$\mathsf{Homotopy\_simple}$ is called instead.  Hence, one applies
Proposition~\ref{prop:compute_regular} yields a runtime
$\softO(m \,n^2 c^2 + n\, (m\sigma'+n^2)\,c\,e )$ operations in
$\KK$. Using again $m \leq n+ \binom{q}{p}\leq n\binom{q}{p}$,
$\sigma'=O(\sigma + {q \choose p} n^3
+n(n\gamma+q\delta))$ and
simplifications which are similar to the ones obtained above, we
obtain as a bound
\[
  \softO\left (
    {q \choose p}\,n^3\, c^2 + n^2\, {q \choose p}^2n^2 \, c\, e (\sigma+ n +\gamma +q \delta)
  \right )
\]
...
but with $X_0=1$. Explicitly, we have
...
simple isolated points) of $\VpFG{p}{\mF}{G}$, with
$$\VpFG{p}{\mF}{G} = \{\bx \in \KKbar{}^n \mid \mathrm{rank}(\mF({\bx})) < p
\text{~and~} g_1(\bx)=\cdots=g_s(\bx)=0\}.$$

We are now interested in designing algorithms for computing the
isolated points or the simple isolated points of $\VpFG{p}{\mF}{G}$
...
We start by stating our first result on computing the simple isolated
points of $\VpFG{p}{\mF}{G}$.

\begin{proposition}\label{prop:rowdegree_simple}
  Suppose that the matrix $\mF \in \KK[X_1,\dots,X_n]^{p \times q}$
  and the polynomials $G=(g_1,\dots,g_s)$ in $\KK[X_1,\dots,X_n]$ are
  given by a straight-line program of length $\sigma$.  If all
  $\gamma_i$'s and $\alpha_j$'s are at least equal to $1$, there
  exists a randomized algorithm that computes the simple isolated
  points of $\VpFG{p}{\mF}{G}$ using
  $$\softO\left( \rc^2 n^6{q \choose p}^2 (\rc + \re ( \sigma + nq p
    \alpha +\gamma +n^3 ) ) \right)$$ 
%   $$\softO\left(
%     \rc^2 n^5 {q \choose p}^2 (\re+\rc^5 n)(\sigma + {q \choose p} n + n q p \alpha+ \gamma + n^3)
% %    \rc(\re+\rc^5 )(\sigma + \rc^2)
%   \right)$$
  operations in $\KK$, with
...
We state now the complexity result for computing the simple isolated
points of $\VpFG{p}{\mF}{G}$.

\begin{proposition}\label{prop:rowdegree}
  We reuse the notations and assumptions introduced for
  Proposition~\ref{prop:rowdegree_simple}. Then, the multiplicities of
  the isolated points of $\VpFG{p}{\mF}{G}$ are at most
  $\rc=\gamma_1\cdots\gamma_s S_{n-s}(\delta_1, \ldots, \delta_q)$.

  Also, there exists a randomized algorithm that computes the 
  isolated points of $\VpFG{p}{\mF}{G}$ using
  $$\softO\left(
    \rc^2 n^5 {q \choose p}^2 (\re+\rc^5 n)
    (\sigma + {q \choose p} n + n q p \alpha+ \gamma + n^3)
%    \rc(\re+\rc^5 )(\sigma + \rc^2)
  \right)$$
%  $$\softO\left( \rc^2 n^6{q \choose p}^2 (\rc + \re ( \sigma + nq p
%    \alpha +\gamma +n^3 ) ) \right)$$
...
These propositions complete the proof of Theorems~\ref{theo:1},
\ref{theo:2} and~\ref{theo:3}. % We start by solving
% Problem~\eqref{problem3} and hence give the algorithm
% $\mathsf{Rowdegree\_simple}$ on which
% Proposition~\ref{prop:rowdegree_simple} relies.

\subsection{Setting up the homotopy and sum of the multiplicities}
...
\paragraph{Properties $\assG_1$, $\assG_2$ and $\assG_3$}
...
\subsection{Towards homotopy algorithms}


Since $\assA_1$, $\assA_2$, $\assG_1(0)$, $\assG_2(0)$ and
$\assG_3(0)$ hold, we are going to apply
Proposition~\ref{prop:compute_regular} to compute the simple isolated
points in $\VpFG{p}{\mF}{G}$. Recall that the resulting algorithm
$\mathsf{RowDegree\_simple}$ is used by the
$\mathsf{RowDegreeDiagonal}$ of previous Section. Next, we will apply
...
$O(n(\gamma_1 + \cdots + \gamma_s))$ operations, we can also compute
...
$\sigma'=\sigma + \sigma_\mN + O({q \choose p} n^3 + n(\gamma_1 +
\cdots + \gamma_s))=\sigma +O( {q \choose p} n^3 + n^2
(\alpha_1+\cdots+\alpha_p) + n(\gamma_1 + \cdots + \gamma_s))$
operations.


\subsection{Cost analysis for $\mathsf{RowDegree\_simple}$}

The algorithm $\mathsf{RowDegree\_simple}$ that we deduce from the
above discussion is given hereafter. We indicate the arithmetic costs
...
\hfill Length of $\Delta$ is $O(n^2(\alpha_1+\cdots+\alpha_p))$
\item for any sequence $\bu=(u_1,\dots,u_s)$, with $u_j \in \{1,\dots,\gamma_j\}$ for all $j$
\begin{enumerate}
\item apply Gaussian elimination to the system of linear forms
  ...
  $(\phi_{\bu,k})_{n-s+1 \le k \le n}$ in $(X_1,\dots,X_{n-s})$
...
\item construct a straight-line program $\Delta_{\bu}$ that computes the matrix 
  ...
  \hfill Length of $\Delta_{\bu}$ is $O(n^2(\alpha_1+\cdots+\alpha_p))$

\item $\scrR'_\bu \gets \mathsf{RowDegreeDiagonal}(\Gamma_\bu)$ (points have coordinates $(X_1,\dots,X_{n-s})$
  
\hfill  $\text{\sf{Cost:~}} \softO(\gamma_1 \cdots \gamma_s T)$, for $T$ as in~\eqref{eqdef:T}

\item deduce $\scrR_\bu$ from $\scrR'_\bu$ by adding the expressions for $(X_{n-s+1},\dots,X_n)$
  
\hfill  $\text{\sf{Cost:~}} O( n^2 \rc)$, with $\rc= \gamma_1 \cdots \gamma_s S_{n-s}(\alpha_1,\dots,\alpha_p)$
\end{enumerate}
\item combine all $\scrR_\bu$ into $\scrR$

  \hfill $\text{\sf{Cost:~}} \softO(n \rc)$

\item construct a straight-line program $\Gamma'$ that computes all polynomials $\bB$

  \hfill Length of $\Gamma'$ is $\sigma'=O(\sigma + {q \choose p} n^3 + n^2
(\alpha_1+\cdots+\alpha_p) + n(\gamma_1 + \cdots + \gamma_s))$

\item\label{step:final:rowdegreesimple} return $\mathsf{Homotopy\_simple}(\Gamma',\scrR)$

  \hfill $\text{\sf{Cost:~}} \softO( \rc(\re+\rc^5 )(\sigma + \rc n^4) )$, 
with $\re=(\gamma_1+1) \cdots (\gamma_s+1) S_{n-s}(\alpha_1+1,\dots,\alpha_p+1)$
\end{enumerate}
\label{RowHom_simple}
\end{algorithm}

To estimate its complexity, we finally apply
Proposition~\ref{prop:compute_regular}, which gives a runtime of
$\softO(\rc^2 m n^2 + \rc \re (\sigma' m + n^2))$ operations in~$\KK$
for the cost of calling the homotopy subroutine at the last step of
Algorithm {\sf RowDegree\_simple}. This runtime is dominated by
  \begin{equation}
    \label{eq:regular}
  \softO\left (\rc m n^2 (\rc + \re \sigma') \right ).     
  \end{equation}


Recall that
$\sigma' = \sigma +O( {q \choose p} n^3 + n^2
(\alpha_1+\cdots+\alpha_p) + n(\gamma_1 + \cdots + \gamma_s))$ and
$s \leq n$. Letting $\alpha = \max ( \alpha_1, \ldots, \alpha_p)$ and
$\gamma = \max(\gamma_1, \ldots, \gamma_s)$, we obtain after some
extra simplifications
\[
  \sigma ' \leq \sigma + n^3 O\left ( {q \choose p} + p\alpha
    +\gamma\right ).
\]

Plugging the above right hand side  in \eqref{eq:regular}, this bound can be
simplified as
\[
\softO\left (\rc m n^2 (\rc + \re ( \sigma + n^3 {q \choose p} +p\alpha +\gamma  ) ) \right ).    
\]
Using  $m\leq n + {q \choose p}$, the above can be bounded by
\[
\softO\left (\rc n^3{q \choose p}^2 (\rc + \re ( \sigma + n^3 + p \alpha +\gamma ) ) \right ).    
\]

% We next show that the three quantities $ {q \choose p}$,
...
% $\softO\left (\rc(\re+\rc^5 )(\sigma + \rc n^4)\right).$
...
\begin{align}\label{eq:recT}
  ...
  \softO\left ( \rc n^6{q \choose p}^2 (\rc + \re ( \sigma + p \alpha +\gamma ) ) \right ),  
\end{align}
...
\begin{lemma}\label{lemma:TMrow}
  One can take
  $T_{M,{\rm row}}((\alpha_1,\dots,\alpha_p),q)= \softO( u^2 \mu^4{q
    \choose p}^2 (u + v ( \mu q p \alpha + \mu^3 +\gamma ) ) )$, with
  $\mu=q-p+1$, $u=S_{\mu}(\alpha_1,\dots,\alpha_p)$ and
  $v=S_{\mu}(\alpha_1+1,\dots,\alpha_p+1)$.
\end{lemma}
\begin{proof}
  We apply the previous formula in the case where $s=0$, so that
...
\\ + \softO\Big(
u \mu^3{q \choose p}^2 (u + v ( \mu q p \alpha + \mu^3 +\gamma ) )
\Big), \quad \mu=q-p+1,\label{rec:Trow}
  \end{multline}
  with $u=S_{\mu}(\alpha_1,\dots,\alpha_p)$ and
  $v=S_{\mu}(\alpha_1+1,\dots,\alpha_p+1)$.  The right-hand term $\softO(
  u \mu^3{q \choose p}^2 (u + v ( \mu q p \alpha + \mu^3 +\gamma ) )
  )$ is at its maximum at
...
+ C,
  \end{align}
  for some constant $C$, and substituting $C=\softO(
  u \mu^3{q \choose p}^2 (u + v ( \mu q p \alpha + \mu^3 +\gamma ) )
  )$.  Now, a quick induction shows
  that $T_{M,{\rm row}}= C \mu S_\mu(\alpha_1,\dots,\alpha_p)$ is a
  solution of~\eqref{rec:TrowC}. As a result,
  $T_{M,{\rm row}}= \softO(
  u^2 \mu^4{q \choose p}^2 (u + v ( \mu q p \alpha + \mu^3 +\gamma ) )
  )$
...
$$T_{\rm row}(\sigma,\bgamma,\balpha,q)=
\softO\Big (
\rc^2 n^5{q \choose p}^2 (\rc + \re ( n q p \alpha + n^3 +\gamma ) ) +
\rc n^6{q \choose p}^2 (\rc + \re ( \sigma + p \alpha +\gamma ) )
\Big ).$$
This can be simplified to
\[
\softO\Big (
%\rc^2 n^5{q \choose p}^2 (\rc + \re ( n q p \alpha + n^3 +\gamma ) ) +
\rc^2 n^6{q \choose p}^2 (\rc + \re ( \sigma + nq p \alpha +\gamma +n^3 ) )
\Big )
\]
which establishes Proposition~\ref{prop:rowdegree_simple}



\subsection{Cost analysis for $\mathsf{RowDegree}$}

Algorithm $\mathsf{RowDegree}$ is similar to
$\mathsf{RowDegree\_simple}$: the only difference consists in calling
algorithm $\mathsf{Homotopy}$ on which
Proposition~\ref{prop:compute_isolated} at the last step
\eqref{step:final:rowdegreesimple} (instead of
$\mathsf{Homotopy\_simple}$).

Hence, reusing the notation of the previous Subsection, as well as the
runtime estimate given in Proposition~\ref{prop:compute_isolated}, the
total cost of algorithm $\mathsf{RowDegree}$ is
  \begin{equation}
    \label{eq:Trowpassimple}
\gamma_1 \cdots\gamma_s T +
  \softO\left ( \rc^5 m n^2  + \rc(\re+\rc^5 n)(\sigma' + n^3) \right ),      
  \end{equation}
where
\begin{itemize}
\item $T$ is as in ~\eqref{eqdef:T};
\item $m \leq n+ {q \choose p}$ and $s \leq n$;
\item $\sigma' = \sigma +O( {q \choose p} n^3 + n^2
(\alpha_1+\cdots+\alpha_p) + n(\gamma_1 + \cdots + \gamma_s))$. 
\end{itemize}
Reusing Lemma~\ref{lemma:TMrow} for bounding $T$ in
\eqref{eq:Trowpassimple}, we obtain that the total cost of algorithm
$\mathsf{RowDegree}$ can be bounded by
\[
  \softO\Big(
  \rc^2 n^5 {q \choose p}^2 (\rc + \re ( n q p \alpha + n^3 +\gamma ) ) +
  \rc^5 n^3 {q \choose p} + \rc n^2(\re+\rc^5 n)(\sigma + {q \choose p} n + p \alpha+ \gamma + n)
  \Big)
\]
...
\[
  ...
  \Big).
\]
This finishes to establish Proposition~\ref{prop:rowdegree}.
