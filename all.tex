...
\begin{theorem}\label{theo:2}
  ...
  Then, there exists a randomized algorithm that solves
  Problem~\eqref{problem2} in either
   $$\softO\left (
     {q \choose p} c(e+c^5 )\big(\sigma +  \max(\deg(g_i), 1\leq i \leq s) + q\max(\cdeg(\mF,i), 1\leq i \leq q) \big )
   \right)$$
  operations in $\KK$, with
  \begin{align*}
    c&=\deg(g_1)\cdots\deg(g_s)\ E_{n-s}(\cdeg(\mF,1), \ldots, \cdeg(\mF,q))\\
    e&=(\deg(g_1)+1)\cdots(\deg(g_s)+1)\ E_{n-s}(\cdeg(\mF,1)+1, \ldots, \cdeg(\mF,q)+1),
    %% \chi&=  \max(\deg(g_i), 1\leq i \leq s) + q\max(\cdeg(\mF,i), 1\leq i \leq q)
  \end{align*}
  or 
  $$\softO\left(
...
\begin{theorem}\label{theo:3}
  ...
  $$\softO\left ( {q \choose p}c e \big(\sigma +  \max(\deg(g_i), 1\leq i \leq s) + q\max(\cdeg(\mF,i), 1\leq i \leq q) \big )\right)$$
  operations in $\KK$, with
  \begin{align*}
  c&=\deg(g_1)\cdots\deg(g_s)\ E_{n-s}(\cdeg(\mF,1), \ldots, \cdeg(\mF,q))\\
    e&=(\deg(g_1)+1)\cdots(\deg(g_s)+1)\ E_{n-s}(\cdeg(\mF,1)+1, \ldots, \cdeg(\mF,q)+1),
...
\begin{proposition}\label{prop:compute_isolated}
  ...
  randomized algorithm which computes a zero-dimensional
...
$\scrR_0$, that is, $\softO((mn^2+n\sigma)c^2)$ operations in $\KK$.
...
$e \le c^5 n$; since one easily checks that $e \ge 2^n$, this implies that $c \ge 2^n/n$, so
that $n$ is in $O(\log(c))$, and $e+c^5 n$ is in $\softO(c^5)$. A
last factorization shows that the bound can be
simplified to
\[
  \softO\left ({q \choose p}  c(e+c^5 )n^3(\sigma +\gamma + q
    \delta)\right).
\]
Noticing that $n\le\log(e)$ then ends the proof of Proposition~\ref{prop:coldeg}.
...
\hfill $\text{\sf{cost:~}} \softO\left (c^5 m n^2 + c(e+c^5n )(\sigma' + n^3)\right)$, 
...
We start by stating our first result on computing the simple
...
Proposition~\ref{prop:compute_regular} to compute the simple
