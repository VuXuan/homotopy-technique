...
\begin{proposition}
  ...
  $\mF \in \KK[X_1,\dots,X_n]^{p \times q}$ and polynomials
  $G=(g_1,\dots,g_s)$ in $\KK[X_1,\dots,X_n]$ are given by a
  straight-line program of length $\Lambda$. Then, there exist 
an algorithm that computes the isolated points of
  $\VpFG{p}{\mF}{G}$ using $$\softO\left (c(e+c^5 )(\Lambda + cn^4)\right)$$
...
program $\Gamma$ of size $\Lambda$ that computes polynomials $G=(g_1,\dots,g_s)$ and
the entries of $\mF$.
...
(\gamma_1+\cdots+\gamma_s))$ operations for $(a_1,\dots,a_s)$, and $O(\Lambda+n (\gamma_1+\cdots+\gamma_s))$ for $(b_1,\dots,b_s)$. 
...
the entries of $\mU$ can be computed in $O(\Lambda +
...
program $\Gamma'$ of size $\Lambda'=O(\Lambda + {q \choose p} n^3 +n(\gamma_1+\cdots+\gamma_s+\delta_1+\cdots+\delta_q))$.

\paragraph{Completing the cost analysis.}
We can then finally apply Proposition~\ref{prop:compute_isolated},
whose runtime is $\softO(c^5 m n^2 + c(e+c^5 n)(\Lambda' + n^3))$ operations
in~$\KK$; since $m \le n + {q \choose p}$, this can be simplified as
$$\softO\left (c(e+c^5 n)\left(\Lambda + {q \choose p} n^3
...
\end{itemize}
As a result, our bound becomes $\softO\left (c(e+c^5 n)(\Lambda +
cn^4)\right).$  This can also be rewritten as $\softO\left
(c(e+c^5 )(\Lambda + cn^4)\right)$, by means of a 
...
{\bf Input}: a straight-line program $\Gamma$ of length $\Lambda$ that computes 
\begin{itemize}  
\setlength\itemsep{0em}
...
\item construct a straight-line program $\Gamma'$ that computes all polynomials $\bB$\\
  length of $\Gamma'$ is $\Lambda'=O(\Lambda + {q \choose p} n^3 + n
(\alpha_1+\cdots+\alpha_p) + n(\gamma_1 + \cdots + \gamma_s))$

\item return $\mathsf{Homotopy}(\Gamma',\scrR)$ \\
$\text{\sf{cost:~}} \softO\left (c(e+c^5 )(\Lambda + cn^4)\right)$, 
