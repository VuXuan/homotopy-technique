...
\item the projective algebraic sets defined in $\P^N(\KKbar)$ by
  $I_p(\mN)$ and $I_p(\mM)$ have no solution at infinity (that is,
  with $Y_0=0$);
...
We next discuss how to find the points $\by\in\P^{N}(\KKbar)$ where
$\mN$ has rank less than~$p$.
\begin{lemma}
  Let $\by\in\P^{N}(\KKbar)$. Then, $\mN$ has rank less than~$p$ if
  and only if there exists a subsequence $\bi=(i_1,\dots,i_\kappa)$ of
  $(1,\dots,p)$ of length $\kappa\in \{1,\dots,\min(N,p)\}$, such that
  $\lambda^H_{i_1,i_1}(\by) = \cdots =
  \lambda^H_{i_\kappa,i_\kappa}(\by) = 0$ and
  $\mN_{\bi;(p+1,\dots,q)}$ has rank less than $\kappa$ at $\by$.
\end{lemma}
% \begin{proof}
...
is to prove that $c=S_{n-s}(\alpha_1,\dots,\alpha_p)$.
