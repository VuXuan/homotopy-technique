...
sets defined over $\KK$ \cite[xxx]{Shafarevich77}.
...
runtime. One crucial information that can be exploited is the
\emph{multiplicity} of $\bx$ as a root of $\bC$. To explain what it
is, we need to introduce some material on algbera and polynomial
systems.




However, when knowing a bound $\mu$ on the \emph{multiplicity} of
$\bx$ yields allows to do better. In Section~\ref{sec:isolated}, we
revisit an algorithm and its complexity analysis which, decides given
a polynomial system $\bC$ and a point $\bx$




and an integer $\mu$ such and decides if the point
$\bx$ is an isolated solution 

In the sequel, we denote by $V(\bC)$ the zero-set of $\bC$ and by
$\langle \bC\rangle$ the ideal it generated. For any prime ideal $P$,
$\langle \bC\rangle_P$ denotes the localized ideal. 
