...
\begin{proposition}\label{prop:compute_regular}
  ...
  $V(\bC)$ using
$$O(m \,n^2 c^2 + n\, (\sigma+n^2)\,c\,e )\subset (e\,\sigma\,m)^{O(1)}$$ operations in~$\KK$.
\end{proposition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
points at which the Jacobian matrix associated to $\bC$ is not
full rank. Doing that is easier than discarding those points which
are not isolated. It suffices to construct a straight-line program
evaluating that Jacobian matrix ; this can be done using
Baur-Strassen's algorithm \cite{BaurStrassen} which yields a
straight-line program of length $\sigma'\in O(m \, \sigma)$ operations
in $\KK$. Next, one evaluates this matrix as done previously when we
were decomposing $\scrR_0$ and use Gauss elimination to identify
divisors of $w_1$ ; the total cost of this step is
$\softO( (mn^2 + n\sigma) c^2)$ operations in $\KK$.

Finally, summing all the costs and taking into account the inequality
$e\geq c$ (Lemma~\ref{lemma:e-geq-c}) ends to prove
Proposition~\ref{prop:compute_regular}. {\todo \bf to be precised.}
...
\begin{proposition}\label{prop:coldeg}
  ...
  Besides, letting $\gamma=\max(\gamma_1, \ldots, \gamma_s)$ and
  $\delta=\max(\delta_1, \ldots, \delta_q)$ there exists randomized
  algorithm that computes these simple isolated points using 
$$ \softO\left ( 
  \left (n+{q \choose p}\right )n^2\, c^2\, +\, 
  n\left ( \sigma+{q \choose p} n^3 (\gamma+\delta)\right ) c\, e\right ) 
$$ 
operations in $\KK$. 
\end{proposition}
...
runtime $\softO(m \,n^2 c^2 + n\, (\sigma'+n^2)\,c\,e )$ operations in
...
$$\softO\left ( 
\left (n+{q \choose p}\right )n^2\, c^2\, +\, 
n\left ( \sigma+{q \choose p} n^3 (\gamma+\delta)\right )  c\, e
\right ).$$
