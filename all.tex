...
In what follows, $\KK$ is a field, $(X_1, \ldots, X_n)$ is a set of
$n$ variables and $\KK[X_1,\dots,X_n]$ is the multivariate polynomial
ring with coefficients in $\KK$. Let $\mF=[f_{i,j}] \in
...
describe the set $S_\mF$ of points $\x \in \KKbar{}^n$ at which the
evaluation of the matrix $\mF$ has rank less than $p$. In the
particular case $p=1$, this simply means finding all common solutions
of $f_{1,1},\dots,f_{1,q}$; in general, $S_\mF$ is an algebraic set,
since it is defined by the vanishing of all maximal minors of $\mF$.
...
least (and, expectedly, equal to) $n-(q-p+1)$; indeed, in the
case $n = q-p+1$, $S_\mF$ is finite for a generic choice of $\mF$ (we
will make this statement more precise in the following
sections). However, beyond the inequality $n = q-p+1$, we make no
...
question, where we solve determinantal systems of an algebraic set. In
addition to matrix $\mF$, we may indeed take into account algebraic
equations of the form $G=(g_1,\dots,g_s)$ in $\KK[X_1,\dots,X_n]$. In this
setting, the natural relation between the number $n$ of variables,
the size of $\mF$ and the number $s$ of polynomials in $G$ is 
now $n=q-p+s+1$. Then, we define the algebraic set
...
For any matrix $\mF$ over a ring $R$, and for any integer $m$,
...
zero-set of $I_r(\mF)$ in $\KKbar{}^n$. Our first result is 
then the following.
...
Nie and Ranestad proved (in a homogeneous context) that the bounds in
Theorem~\ref{theo:1} are equalities for two families of polynomials:
if the entries of $\mF$ are chosen generic and such that
$\deg(f_{i,j}) = \rdeg(\mF,i)$ for all $i,j$, then the ideal $I_{\mF}$
has degree $\delta_{\rm row}(\mF)$; when the entries of $\mF$ are
chosen generic and such that $\deg(f_{i,j}) = \cdeg(\mF,j)$ for all
$i,j$, the ideal $I_{\mF}$ has degree $\delta_{\rm col}(\mF)$. From
this, they deduce that the degree of the ideal $I_{\mF,G}$ is at most
$\deg(g_1) \cdots \deg(g_s) \delta_{\rm row}(\mF)$, in the particular
case when the matrix $\mF$ is the Jacobian matrix of $G$ (together
with one extra row, corresponding to the gradient of a function $q$
that we want to optimize on $V(G)$), and assuming that this ideal has
dimension zero.
...
where $T$ is a new variable. However, we need some properties on the
start matrix such that we can compute the isolated solutions for the
determinantal system of $F$. Assume that we know the solutions for the
determinatal system of $G$, by using the Newton interation to the
determinantal system of $H$, we can lift into the isolated solutions
for that of $F$. Therefore, we need create a start matrix $G$ such
that
\begin{itemize}
...
\begin{proposition}\label{prop:testisolated}
  ...
  isolated root of $V(\bC)$ using $(\mu\,E\,m)^{O(1)}$ operations in~$\KK$.
\end{proposition}
...
discuss this in Section~\ref{sec:check}.  For $\tau$ in $\KKbar$, we
further denote by $\assG(\tau)$ the following three properties.
\begin{description}[leftmargin=*]
...
\item[${\assD}_2$.] Given $\bx \in V(\bB_0)$ having coordinates in a field
  extension $\LL$ of $\KK$, we can find in time $(D [\LL:\KK])^{O(1)}$ a sequence $\bi_\bx=(i_1,\dots,i_n)$, with
$1 \le i_1 < \dots < i_n \le m$, such that the
  Jacobian matrix of $(B_{0,i_1},\dots,B_{0,{i_n}})$ has full
  rank $n$ at $\bx$, for some $D$ independent of $\bx$.
\item[${\assD}_3$.] We know an integer $e$ such that the curve $V(J')$
  has degree at most $e$.  \todo{define $J'$}
\item[${\assD}_4$.] We can compute $\bB$ using a straight-line program
  of length $E$.
\end{description}
Then, the second result in this section is the following.
\begin{proposition}\label{prop:compute_isolated}
 ...
 of $V(\bC)$ using $(D\,e\,E\,m)^{O(1)}$ operations in~$\KK$.
\end{proposition}
...
The rest of this subsection is devoted to prove
...
\begin{proof}
  ...
  B_k$ satisfy a relation of the form $h^H_k = B_{0,k}^H +
...
nonzero minor. Let $\bB'=(B_1,\dots,B_n)$.
...
$$\bB'(\Phi_i) = \bB'(f) + \jac_f(\bB',\bX) T^m \delta_i + T^{2m} r_i =0$$
and
$$\bB'(\Phi_{i'}) = \bB'(f) + \jac_f(\bB',\bX) T^m \delta_{i'} + T^{2m}
r_{i'} =0,$$ for some vectors of bounded Puiseux series $r_i,r_{i'}$.
By subtraction and division by $T^m$, we obtain $\jac_f(\bB',\bX)
...
zero-dimensional parametrization of $V(\bB_{0})$ obtained by 
means of assumption $\assD_1$.
...
$\bi_\bx=\bi_j$ for all $\bx$ in $Z(\scrR_{0,j})$. In other words, for
all points described by $\scrR_{0,j}$, the jacobian of $(B_{0,i})_{i
  \in \bi_j}$ has full rank $n$ at $\bx$, for all $\bx$ in
$Z(\scrR_{0,j})$. This is done by applying the algorithm of assumption
${\assD}_2$ to the point $(v_{0,1}/q_0',\dots,v_{0,n}/q_0')$ that has
...
r_0)\bmod s_0),\lambda)$, with $s_0=q_0/r_0$ \todo{check the
  formulas!}, that define a partition of $Z(\scrR_0)$ into the
subsets where $r_0$ vanishes, resp.\ is non-zero. 

We can then start over again, from $\scrR'_0$ and $\scrR''_0$
independently. The runtime assumption in ${\assD}_2$ implies that the
overall time spent in this step is $(D c)^{O(1)}$.
...
in $\KK[[T]]/\langle T^{2e}\rangle$, for $e$ as in ${\assD}_3$.
...
solutions of $\bC$ (in the cost analysis below, we simply use $e$, 
since $e \ge c$).
...
is $(e\,E\,m)^{O(1)}$ operations in~$\KK$, where $E$ is from
$\assD_4$. Adding all costs seen so far, we prove
...
\section{Column-degree homotopy for determinantal systems}
...
\begin{proposition}
  ...
  points of $S_{\mF,G}$ using $XXX$ operations in $\KK$.
\end{proposition}
...
$$\ell_j = \prod_{k=1}^{\delta_j}\lambda_{j,k},$$
and we let  $\mL$ be the matrix
\[\mL = 
\left( \begin{matrix}
\ell_1 & 2\ell_2 & \cdots & q\ell_{q}\\
\ell_1 & 2^2\ell_2 & \cdots & q^2\ell_q\\
\vdots & \vdots &  & \vdots \\
\ell_1 & 2^p\ell_2 & \cdots & q^p\ell_q
...
the polynomial $a_i$ is equal to $c_i\ell_{j_{i,1}}\cdots
\ell_{j_{i,p}}$, where $c_i$ is a non-zero constant, so it has degree
...
$$a_i^H=\prod_{k=1}^{\gamma_i} (\mu_{i,k,0}X_0 + \sum_{\ell = 1}^{n}\mu_{i,k,\ell}X_\ell),$$
whereas for $i=s+1,\dots,m$, 
$$a_i^H=c_i \ell^H_{j_{i,1}}\ldots \ell^H_{j_{i,p}}, \quad \text{~for~} \bj_i=(j_{i,1},\dots,j_{i,p}) \text{~as above},$$
where for $j=1,\dots,q$ we set 
$\ell^H_j = \prod_{k=1}^{\delta_j}\lambda^H_{j,k}$,
with
...
After applying this substitution, for all $j=1,\dots,q$, $\ell^H_j$ can be rewritten as
$$\ell^H_{j,\bu}=\prod_{k=1}^{\delta_j}\lambda^H_{j,k,\bu},$$
where 
$$\lambda^H_{j,k,\bu}=\lambda_{j,k,0}X_0 + \sum_{\ell =
  1}^{n-s}\lambda_{j,k,\ell}X_k + \sum_{\ell =
...
exactly $p-1$ columns not indexed by $\bj=(j_1,\dots,j_{n-s})$; call
them $\bj'=(j'_1,\dots,j'_{p-1})$. We can then consider the 
products
$$ \ell_{j_1} \ell_{j'_1} \cdots \ell_{j'_{p-1}},\dots, \ell_{j_{n-s}}
\ell_{j'_1} \cdots \ell_{j'_{p-1}};$$ each of them (up to a non-zero
...
polynomials are equal to the linear forms
...
\paragraph{Property $\assD_2$.} Given $\bx$ in $V(\bA)$,
corresponding to indices $\bu$, $\bj$ and $\bv$, we saw in the proof
of $\assG_3$ how to find $n-s$ $p$-minors
$(a_{e_1},\dots,a_{e_{n-s}})$ of $\mA$ such that
$(g_1,\dots,g_s,a_{e_1},\dots,a_{e_{n-s}})$ is a unit times a linear
system, in the local ring at $\bx$. Thus, we have established
$\assD_2$, with $T=O(1)$. \todo{change $T$}

\paragraph{Property $\assD_3$.} Next, we need to determine an upper bound 
...
\paragraph{Property $\assD_4$.} Finally, we need to give an estimate on
...
The polynomials $\ell_1,\dots,\ell_q$ can be computed in $O(n \sum_j
...
program of size
$O(E + {q \choose p} n^3 +n \sum_i \gamma_i + n \sum_j \delta_j)$.

%% \begin{lemma} \label{G} 
%% Let G be the matrix as we define above and $\g$ be the ideal generated
%% by $p \times p$ minors of $G$. Then, $\g$ has exactly solutions and there is an algorithm, namely Algorithm
%% \ref{StartMatCol}, to compute these solutions in $(nE_{n}(\delta_1, \ldots,
%% \delta_q))^{O{1}}$ operations in $\KK$.
%% \end{lemma}
%% \begin{proof}
...
%% &=& \sum_{(i_1, \ldots, i_{n}) \subset \{1, \ldots,q\}^{n}, \ i_j \ne i_k}\prod_{j =1}^{n}\delta_{i_j} = E_{n}(\delta_1, \ldots, \delta_q). 
%% \end{eqnarray*}

%% For the complexity, since each polynomial $L_{i,j}$ is linear, so the time to solve the system in the step $2.i$ is $n^{O{1}}$; and there are $E_{n}(\delta_1, \ldots, \delta_p)$ systems which we need to solve. Therefore, the complexity of the Algorithm $\ref{StartMatCol}$ is $(nE_{n}(\delta_1, \ldots, \delta_q))^{O{1}}$.
...
%% \label{StartMatCol}
%% {\bf Input}: a matrix $G \in \KK[\bX]^{p \times q}$ which is defined as above.\\
%% {\bf Output}: $E_{n}(\delta_1, \ldots, \delta_q)$ solutions of $\g$. 

%% \begin{enumerate}
%% \item $S \gets \emptyset$
%% \item for any $(i_1, \ldots, i_{n}) \in \{1, \ldots, q \}^{n}$: 
%% \begin{itemize}
%% \item for any $(j_1, \ldots, j_n) \in \{1, \ldots, \delta_{i_1}\} \times \cdots \times \{1, \ldots, \delta_{i_n}\} $: 
%% \begin{enumerate}
...
For any ring $R$ and any matrix $\mM \in A^{p\times q}$, if $S$ is a
subsequence of $(1,\dots,p)$ and $T$ a subsequence of $(1,\dots,q)$,
the $(S,T)$-submatrix of $\mM$ is the submatrix obtained by keeping
