...
%% $\balpha=(\alpha_1,\dots,\alpha_p)$ be positive integers.
...
%% indeed, for $\kappa > t$, a system of the 
%% form $ \lambda^H_{i_1,i_1}(\tilde\bx) =
%% \cdots = \lambda^H_{i_\kappa,i_\kappa}(\tilde\bx) = 0$ 
%% has no solution for generic choices of the linear forms $\lambda^H_{i,j,k}$.

%% In turn, $\lambda^H_{i_1,i_1},\dots,\lambda^H_{i_\kappa,i_\kappa}$
%% vanish at $\tilde\bx$ if and only if there exist
%% $\br=(r_1,\dots,r_\kappa)$, with $r_m$ in $\{1,\dots,\alpha_m\}$ 
%% for all $m$, such that $\lambda^H_{i_m,i_m,r_m}(\tilde\bx)=0$ 
%% for $m=1,\dots,\kappa$. For a generic choice of the coefficients 
...
%% on the choice of indices $\bi$ and $\br$. Applying these substitutions
%% in the submatrix $\mN^H_{\bi;(p+1,\dots,q)}$ gives us a matrix
%% $\mM^H_{\bi,\br}$ of size $\kappa \times (t-1)$ whose entries are
%% products of homogeneous linear forms in $X_0,\dots,X_{t-\kappa}$, of
%% respective degrees $\alpha_{i_1},\dots,\alpha_{i_\kappa}$ on rows
%% $1,\dots,\kappa$.

%% We deduce that the projective set defined by $I_p(\mN^H)$ is the union
%% of the sets
%% $$V_{\bi,\br}=\{
%% (x_0,\dots,x_{t-\kappa},f_{\bi,\br,t-\kappa+1}(x_0,\dots,x_{t-\kappa}),\dots,
...
%% $(i_m,r_m)$, $m=1,\dots,\kappa$, this linear form does not vanish on
%% $V_{\bi,\br}$.
...
%% following pseudo-code summarizes the discussion above. \todo{write it}
...
%% $S_{m,m'}$ be the set of all subsequences $\bi=(i_1,\dots,i_\kappa)$
%% of $(1,\dots,p)$ of cardinality $\kappa \in \{m,\dots,m'\}$. 
...
\end{description}
The first step of the proof is to establish that for $\balpha$ and $q$ as above,
$\assJ_1(\balpha,q)$ implies $\assI_1(\balpha,q)$. Let us thus fix $\balpha$
and $q$.  Assumption $\assJ_1(\balpha,q)$ implies that
$V_{p-1}(\mathfrak{N}_{\balpha,q}(G'))$ is empty for a generic $G'$
in~$\KKbar{}^{N'}$.  We will prove that
$V_{p-1}(\mathfrak{M}_{\balpha,q}(G))$ is empty for a generic $G$
in~$\KKbar{}^N$, which in turn establishes $\assI_1(\balpha,q)$.

Consider the ideal $I_{p-1}(\mathfrak{M}_{\balpha,q})$ in the polynomial
ring $\KK[\mathfrak{G},\tilde\bX]$ in $N+n+1$ variables. This ideal
defines an algebraic set $Z_{\balpha,q}$ in $\KKbar{}^N \times
\P^n(\KKbar)$, and we let $\alpha_{\balpha,q} \subset \KKbar{}^N$ be its
projection on the first factor: this is the set of all $G$ such that
$V_{p-1}(\mathfrak{M}_{\balpha,q}(G))$ is not empty. Because the source is
a projective space, $\alpha_{\balpha,q}$ is closed (so its complement is
open), and we just have to verify that it is not equal to the whole
$\KKbar{}^N$. This follows readily from property $\assJ_1(\balpha,q)$,
which proves that generic matrices of the form
$\mathfrak{N}_{\balpha,q}(G')$ do not belong to $\alpha_{p,q}$, so 
$\assI_1(\balpha,q)$ holds.

We finish the proof by induction. We first take $p=q$ and consider
$\assJ_1(\balpha,q)$.  In this case, $n=1$ and $\mathfrak{N}_{\balpha,q}$ is
...
entries $\mathfrak{N}_{\balpha,q}$ have any common solution in
$\P^1(\KKGpbar)$, so the rank of $\mathfrak{N}_{\balpha,q}$ is at least
$p-1$. Hence, $\assJ_1(\balpha,p)$ holds, and so does $\assI_1(\balpha,p)$, by the claim in the previous paragraph.

Consider next a pair $(\balpha,q)$, with $\balpha=(\alpha_1,\dots,\alpha_p)$ and $2 \le
p \le q$, and suppose that $\assI_1(\balpha',q')$ holds for all
$(\balpha',q')$ with $\balpha'=(D'_1,\dots,D'_{p'})$, $2 \le p' \le q'$, $p'
\le p$ and $q' < q$; we prove that $\assJ_1(\balpha,q)$ holds
(as above, this will also imply $\assI_1(\balpha,q)$).

Take $k=p-1$ in Lemma~\ref{lemma:union}. Then, the parameters
$(t-(p-k),\balpha_s,n-1)$ used in each contribution to~\eqref{eq:union}
...
\begin{enumerate}
\item[1.1] write $X_{t-\kappa+1}, \ldots, X_t$ in linear forms of $X_1, \ldots, X_{t-\kappa}$ from $(\lambda_j)_{j \in \bi}$%$(\lambda_{i_1,i_1}, \ldots, \lambda_{i_\kappa, i_\kappa})$
...
, with $\kappa \le q-p$ \todo{say why}. As a result, we 
