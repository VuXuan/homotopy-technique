...
\item the target system $\bC \in \KK[\bX]^{m}$, a start system $\bA \in \KK[\X]^m$
...
the next section. In our case, we do not need the extra set of equations
 $G$.


\begin{algorithm}[H]
\caption{$\mathsf{RowDegreeDiagonal}$}
{\bf Input}: 
matrix $\mN^H \in \KK[X_0, \ldots, X_n]^{p \times q}$ as in~\eqref{eqdef:type1aff}, with $p \leq q$ and $n = q-p+1$\\
{\bf Output}: a zero-dimensional parametrization $\scrR$ of $V(I_p(\mN))$
\begin{enumerate}
