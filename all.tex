...
together with a more standard homotopy 
\[V = (1-T) \cdot K + T \cdot G\]
where $T$ is a new variable. However, we need some properties on the
start matrix such that we can compute the isolated points of
$S_{\mF, G}$. Assume that we know the solutions for the determinantal
system of $\mL$, by using the Newton iteration to the determinantal
system of $\mU$, we can lift into the isolated solutions for that of
$\mF$. Therefore, we need to create a start matrix $\mG$ such that
\begin{itemize}
...
One difficulty raised by the above homotopy process is  
In Section~\ref{sec:isolated}, we revisit an algorithm and its
complexity analysis which, given a polynomial system $\bC$ and a point
$\bx$
