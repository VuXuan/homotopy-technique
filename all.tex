...
$$a_i^H=\prod_{k=1}^{\gamma_i} \mu^H_{i,k} \quad\text{with}\quad \mu^H_{i,j}=(\mu_{i,k,0}X_0 + \sum_{\ell = 1}^{n}\mu_{i,k,\ell}X_\ell),$$
...
In what follows, we will work with two families of matrices, namely matrices
such as
\begin{align}\label{eqdef:type1}
\left( \begin{matrix}
\ell^H_{1,1} & 0 & \cdots & 0 & \ell^H_{1,p+1} & \cdots & \ell^H_{1, q}\\
0 & \ell^H_{2,2\bu} & \cdots & 0 & \ell^H_{2,p+1} & \cdots & \ell^H_{2, q}\\
\vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \ell^H_{p,p} & \ell^H_{p,p+1} & \cdots & \ell^H_{p, q}
\end{matrix} \right),
\end{align}
and matrices of a more general form such as
\begin{align}\label{eqdef:type2}
 \left( \begin{matrix}
\ell^H_{1,1} & \ell^H_{1,2} & \cdots & \ell^H_{1, q}\\
 \ell^H_{2,1} &  \ell^H_{2,2} & \cdots & \ell^H_{2, q}\\
 \vdots & & & \vdots\\
 \ell^H_{p,1} &  \ell^H_{p,2}& \cdots & \ell^H_{p, q}
\end{matrix} \right).
\end{align}
In both cases, for all $i,j$, the entry $\ell^H_{i,j}$ is a product of
$\alpha_i$ homogeneous linear forms in $N+1$ variables $Y_0,\dots,Y_N$
with generic coefficients (except when $\ell^H_{i,j}$ is explicitly
set to zero in the first case), that is,
$\ell^H_{i,j}=\prod_{k=1}^{\alpha} \lambda^H_{i,j,k}$. In both cases,
the number $N$ is given by $N=q-p+1$.

In appendix, we prove the following properties.
For generic choices of the coefficients of the linear forms
$\lambda_{i,j,k}$,
\begin{itemize}
\item the projective algebraic sets defined by $I_p(\mM_1)$ and $I_p(\mM_2)$ 
  have no solution at infinity
\item the Jacobian matrix of $I_p(\mM_1)$ and $I_p(\mM_2)$ with
  respect to $Y_0,\dots,Y_N$ has rank $N$ at every point of the above
  sets.
\end{itemize}

\paragraph{Setting up the homotopy.}
Again, we are going to rely on the algorithm of
Section~\ref{sec:homotopy}. As in the previous section, we let
$\bC=(c_1,\dots,c_s,\dots,c_m)$ be such that
$(c_1,\dots,c_s)=(g_1,\dots,g_s)$ and $(c_{s+1},\dots,c_m)$ are the
$p$-minors of $\mF$. Our main concern is to design a sequence of
polynomials $\bB=(b_1,\dots,b_m)$ in $\KK[T,\bX]$ such that
$\bB_1=\bC$, such that we can solve efficiently the system
...
could imagine a matrix with $i$th row of the form $[1^i
  \ell_i~\cdots~q^i \ell_i]$, but the corresponding determinantal
variety has positive dimension). Instead, we use a deformation that
cancels out many off-diagonal terms.
...
\ell_{1,1} & 0 & \cdots & 0 & \ell_{1,p+1} & \cdots & \ell_{1, q}\\
0 & \ell_{2,2} & \cdots & 0 & \ell_{2,p+1} & \cdots & \ell_{2, q}\\
\vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \ell_{p,p} & \ell_{p,p+1} & \cdots & \ell_{p, q}
\end{matrix} \right), \] 
where all $\ell_{i,j}$ are products of $D_i$ linear forms with generic
coefficients, of the form
$$\ell_{i,j}= \prod_{k=1}^{\alpha_i}\lambda_{i,j,k},
\quad\text{with}\quad
\lambda_{i,j,k} =\lambda_{i,j,k,0} + \sum_{\ell=1}^n \lambda_{i,j,k,\ell}X_\ell.
$$ Then, as said we define $(b_{s+1},\dots,b_m)$ as the $p$-minors of
...
Indeed, any $p$-minor of $\mL$ is of the form $\ell_{i_1,i_1} \cdots
\ell_{i_\kappa,i_\kappa} \det(\mL_{\bi \mathbf{;}(p+1,\dots,q)})$, for
some sequence $\bi=(i_1,\dots,i_\kappa)$ of length $\kappa \in
\{0,\dots,n\}$. Since the entries of $\mL_{\bi
  \mathbf{;}(p+1,\dots,q)}$ are products of linear form with randomly
chosen coefficients $(\lambda_{i,j,k,\ell})$, for a generic choice of
these coefficients, the determinant of this matrix has degree
$\sum_{i' \notin \bi} \alpha_{i'}$. As a result, the corresponding $p$-minor of $\mL$ 
has degree $\alpha_1 + \cdots + \alpha_p$, as claimed.
...
variable $X_0$, and we consider the system $\bA^H=(a_1^H,\dots,a_m^H)$ obtained 
by homogenizing all equations in $\bA$. Thus we have
$$a_i^H=\prod_{k=1}^{\gamma_i} \mu^H_{i,k} \quad\text{with}\quad \mu^H_{i,j}=(\mu_{i,k,0}X_0 + \sum_{\ell = 1}^{n}\mu_{i,k,\ell}X_\ell)$$
for $i=1,\dots,s$, whereas $a_{s+1},\dots,a_m$ are the $p$-minors of the matrix
\begin{align*}
\mL^H = \left( \begin{matrix}
\ell^H_{1,1} & 0 & \cdots & 0 & \ell^H_{1,p+1} & \cdots & \ell^H_{1, q}\\
0 & \ell^H_{2,2} & \cdots & 0 & \ell^H_{2,p+1} & \cdots & \ell^H_{2, q}\\
\vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \ell^H_{p,p} & \ell^H_{p,p+1} & \cdots & \ell^H_{p, q}
\end{matrix} \right),  
\end{align*}
where $\ell^H_{i,j}$ is the homogenization of $\ell_{i,j}$. The
solutions of $\bA^H$ in $\P^n(\KKbar)$ are found by first solving the
equations $a^H_1,\dots,a^H_s$.

As in the previous section, all $a_i^H$ are products of linear forms,
so any solution of $a^H_1,\dots,a^H_s$ is obtained by setting some of
these linear forms to zero. We choose indices $\bu=(u_1,\dots,u_s)$,
with $u_1\in\{1,\dots,\gamma_1\}$, \dots,
$u_s\in\{1,\dots,\gamma_s\}$, and we solve
$$\mu^H_{i,u_i}=0, \quad \text{~that is,~} \quad \mu_{i,u_i,0}X_0 + \sum_{\ell = 1}^{n}\mu_{i,u_i,\ell}X_\ell =0,$$ for $i=1,\dots,s$.
In what follows, we fix such an $\bu$.
Then, for a generic choice of coefficients $\mu_{i,k,\ell}$, these equations
are equivalent to
...
\begin{align}\label{eqdef:mLH}
 \mL^H_\bu = \left( \begin{matrix}
\ell^H_{1,1,\bu} & 0 & \cdots & 0 & \ell^H_{1,p+1,\bu} & \cdots & \ell^H_{1, q,\bu}\\
0 & \ell^H_{2,2\bu} & \cdots & 0 & \ell^H_{2,p+1,\bu} & \cdots & \ell^H_{2, q,\bu}\\
\vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \ell^H_{p,p,\bu} & \ell^H_{p,p+1,\bu} & \cdots & \ell^H_{p, q,\bu}
\end{matrix} \right),
\end{align}
with
$$\ell^H_{i,j,\bu}=\prod_{k=1}^{\alpha_i}\lambda^H_{i,j,k,\bu},
\quad\text{and}\quad \lambda^H_{i,j,k,\bu}=\sum_{\ell =
  0}^{n-s}\lambda_{i,j,k,\ell}X_\ell + \sum_{\ell =
  n-s+1}^{n}\lambda_{i,j,k,\ell}
\mathfrak{h}_{\ell,\bu}(X_0,\dots,X_{n-s}).$$ At this stage, we are
left with finding the points $\bx'\in\P^{n-s}(\KKbar)$ where
$\mL^H_\bu$ has rank less than~$p$. Remark that this is a $p\times q$
matrix, with entries that are products of linear forms with generic coefficients,
all entries of row $i$ having degree $\alpha_i$.
 The next property is the key of
our recurrence.
\begin{lemma}
  Let $\bx'\in\P^{n-s}(\KKbar)$. Then, $\mL^H_\bu(\bx')$ has rank less than~$p$
  if and only if there exists $\bi=(i_1,\dots,i_\kappa)$ subsequence
  of $(1,\dots,p)$ of length  $\kappa\in \{1,\dots,\min(n-s,p)\}$,
  such that $\ell^H_{i_1,i_1}(\bx') = \cdots = \ell^H_{i_\kappa,i_\kappa}(\bx') = 0$
  and $\mL^H_{\bi;(p+1,\dots,q)}(\bx')$ has rank less than $\kappa$.
\end{lemma}
\begin{proof}
  If $\mL^H_\bu(\bx')$ has rank less than~$p$, then some diagonal
  terms, say $\ell^H_{i_1,i_1},\dots,\ell^H_{i_\kappa,i_\kappa}$, must
  vanish at $\bx'$, all other ones being non-zero. Thus, we must 
  have $\kappa \ge 1$; on the other hand, since the
  coefficients of the linear forms making up the polynomials
  $\ell^H_{i,i,\bu}$ are random, any system made of more than $n-s$ of
  these polynomials has no solution. Hence, $\kappa$ is in
  $\{1,\dots,\min(n-s,p)\}$.
  ...
  the polynomial
  $\ell^H_{i'_1,i'_1,\bu}\cdots\ell^H_{i'_{p-\kappa},i'_{p-\kappa},\bu}\zeta$
  is a $p$-minor of $\mL^H_\bu$, so that $\zeta(\bx')$ must vanish.
  As a result, $\mL^H_{\bi;(p+1,\dots,q)}$ has rank less thank $\kappa$.

  Conversely, if $\ell^H_{i_1,i_1}(\bx') = \cdots =
  \ell^H_{i_\kappa,i_\kappa}(\bx') = 0$ and
