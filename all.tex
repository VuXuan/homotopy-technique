...
\alpha_{i_1} \cdots \alpha_{i_\kappa} T_{\rm row}(E+O(n^2), (), (\alpha_{i_1},\dots,\alpha_{i_\kappa}), q)\right),$$
...
\begin{proof}
  ...
  n^3 + T_{\rm row}(E+O(n^2), (), (\alpha_{i_1},\dots,\alpha_{i_\kappa}), q)\right).$$
  The cost reported in the summand does not depend on $\br$, so that 
  the sum can be rewritten as 
  $$\sum_{\substack{\bi=(i_1,\dots,i_\kappa)\\ \kappa\le\min(n,p)}}
  \alpha_{i_1} \cdots \alpha_{i_\kappa}( n^3 + T_{\rm row}(E+O(n^2), (), (\alpha_{i_1},\dots,\alpha_{i_\kappa}), q)).$$
...
\begin{lemma}
  ...
  function of degree $t$ in $\alpha_1,\dots,\alpha_p$. For a generic
  matrix $\mN$ of size $p \times q$ as in~\eqref{eqdef:type1aff} or a
  generic matrix $\mM$ of the form
  ...
  $\lambda_{i,j,1},\dots,\lambda_{i,j,\alpha_i}$ in $t$ variables
  $X_1,\dots,X_t$, with $t=q-p+1$, $V_p(\mN)$ and $V_p(\mM)$ have
...
$V(\bA)$. This is done by using Algorithm {\sf RowDegreeDiagonal}
...
in time $O(n \alpha_i)$, for a total of $L=O(n^2 \sum_i \alpha_i)$
operations in $\K$
...
takes $O(\gamma_1 \cdots \gamma_s n^3)$ operations in $\KK$.
...
so by Lemma~\ref{lemma:rowdegreediagonal}, this takes time
$$\softO\left (n^3 c  + \sum_{\substack{\bi=(i_1,\dots,i_\kappa)\\ \kappa\le\min(n,p)}}
\alpha_{i_1} \cdots \alpha_{i_\kappa} T_{\rm row}(E+O(n^2), (), (\alpha_{i_1},\dots,\alpha_{i_\kappa}), q)\right).$$
The total cost is obtained by multiplying this 
value by the number $\gamma_1 \cdots \gamma_s$ of choices of $\bu$.
...
%% \gamma_i)$ operations for $(a_1,\dots,a_s)$, and $O(E+n \sum_i
%% \gamma_i)$ for $(b_1,\dots,b_s)$. The polynomials
%% $(b_{s+1},\dots,b_m)$ are the $p$-minors of $\mU=(1-T)\mN+T\mF$.
...
\begin{enumerate}
\item construct 
 matrix $\mN \in \KK[\bX]^{p \times q}$ as in~\eqref{eqdef:type1aff}
...
\item substitute $(X_i)_{n-s+1 \leq i \leq n}$ into $\mN$ to obtain $\mN_\bu\in \KK[X_1, \ldots, X_{n-s}]^{p \times q}$
