...
%% generality further down. We assume that $V(I)\ne
%% \emptyset$, so that $I \ne \KK[T,\bX]$. 
...
an algorithm that computes the isolated points of
  ...
  \delta_q+1)$,
  where $E_{q-p+1}(\cdots)$ is the elementary symmetric 
  function of degree $q-p+1$.
\end{proposition}
...
$O(cn)$ (recall that $c=\gamma_1 \cdots \gamma_s E_{q-p+1}(\delta_1,\dots,\delta_q)$)
\begin{itemize}
...
\begin{proposition}\label{prop:rowdegree}
  ...
  straight-line program of length $\sigma$. Then, there exist 
an algorithm that computes the isolated points of
  $\VpFG{p}{\mF}{G}$ using $$\softO\left(  \right)$$
  operations in $\KK$, with
  $c=\gamma_1\cdots\gamma_s S_{q-p+1}(\delta_1, \ldots, \delta_q)$ and
  $e=(\gamma_1+1)\cdots(\gamma_s+1) S_{q-p+1}(\delta_1+1, \ldots,
  \delta_q+1)$,
  where $S_{q-p+1}(\cdots)$ is the complete homogeneous symmetric 
  function of degree $q-p+1$.
...
\begin{corollary}\label{coro:complete}
  ...
  $V(\bA)$ is $\gamma_1 \cdots \gamma_s S_{p-k+1}(\alpha_1,\dots,\alpha_p)$.
\end{corollary}
\begin{proof}
  ...
  has cardinality $S_{q-p+1}(\alpha_1,\dots,\alpha_p)$; the conclusion
  follows.
\end{proof}
...
The number of points $c$ in the output is $S_{q-p+1}(\alpha_1,\dots,\alpha_p)$
...
+
\softO(n^3 S_{q-p+1}(\alpha_1,\dots,\alpha_p)).  \label{eqdef:T}
\end{eqnarray}
...
\cdots \gamma_s n^2 S_{q-p+1}(\alpha_1,\dots,\alpha_p))$. Finally, we
...
$\softO(\gamma_1 \cdots \gamma_s n S_{q-p+1}(\alpha_1,\dots,\alpha_p))$.

Altogether, the overall time spent in computing the zero-dimensional
parametrization $\scrR$ of $V(\bA)$ is 
$\gamma_1 \cdots \gamma_s\sum_{\substack{\bi=(i_1,\dots,i_\kappa), \kappa\le\min(q-p,p)}}
\alpha_{i_1} \cdots \alpha_{i_\kappa} T_{M,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), q-p)$
$+
\softO(\gamma_1 \cdots \gamma_s n^3 S_{q-p+1}(\alpha_1,\dots,\alpha_p)).$
...
integer $(\gamma_1+1)\cdots(\gamma_s+1) S_{q-p+1}(\alpha_1 +1,\cdots,\alpha_p+1)$.
...
\item The sum $\alpha_1+\cdots+\alpha_p$ is at most equal to
  $S_{q-p+1}(\alpha_1,\dots,\alpha_p)$, and thus to $cn$.
\end{itemize}
...
\begin{lemma}
  ...
  n^5 q u^3(v+u^5 )))$, with $n=q-p+1$,
  $u=S_{q-p+1}(\alpha_1,\dots,\alpha_p)$ and
  $v=S_{q-p+1}(\alpha_1+1,\dots,\alpha_p+1)$.
\end{lemma}
\begin{proof}
  ...
  $\sigma=O(nq(\alpha_1 + \cdots + \alpha_p))$; following our
...
\alpha_{i_1} \cdots \alpha_{i_\kappa} T_{M,{\rm row}}((\alpha_{i_1},\dots,\alpha_{i_\kappa}), n-1)
\\ + \softO\Big(  
  n^4 q u(v+u^5 )(\alpha_1 + \cdots + \alpha_p)\Big), \quad n=q-p+1,\label{rec:Trow}
  \end{multline}
  with $u=S_{q-p+1}(\alpha_1,\dots,\alpha_p)$ and
  $v=S_{q-p+1}(\alpha_1+1,\dots,\alpha_p+1)$.  The right-hand term $\softO(
  n^4 q u(v+u^5 )(\alpha_1 + \cdots + \alpha_p))$ is at its maximum at
...
+ C,
  \end{align}
  for some constant $C$, and substituting $C=\softO( n^4 q u(v+u^5
  ...
  solution of~\eqref{rec:TrowC}. As a result,
   $T_{M,{\rm row}}= \softO( n^5 q u^2(v+u^5 )(\alpha_1 + \cdots + \alpha_p)))$
  is a solution of~\eqref{rec:Trow},
  and this quantity is $ \softO( n^5 q u^3(v+u^5 )))$.
\end{proof}

%% $$\softO(c^5 m n^2  + c(e+c^5 n)(\sigma + n^3))
...
\item return $\mathsf{Homotopy}(\Gamma',\scrR)$\\
 $\text{\sf{cost:~}} \softO( c(e+c^5 )(\sigma + cn^4) )$
\end{enumerate}
\label{Row}
\end{algorithm}
