...
In what follows, we will work with two families of matrices in $\KK[Y_0,\dots,Y_N]^{p\times q}$, namely matrices
such as
...
\end{matrix} \right).
\end{align}
In both cases, for all $i,j$, the entry $\lambda^H_{i,j}$ is a product of
$\alpha_i$ homogeneous linear forms in $N+1$ variables $Y_0,\dots,Y_N$
with generic coefficients (except when $\lambda^H_{i,j}$ is explicitly
set to zero in the first case), that is,
$\lambda^H_{i,j}=\prod_{k=1}^{\alpha} \lambda^H_{i,j,k}$. In both cases,
the number $N$ is given by $N=q-p+1$. In appendix, we prove the following properties.
...
\item the projective algebraic sets defined in $\P^N(\KKbar)$ by
  $I_p(\mN)$ and $I_p(\mM)$ have no solution at infinity (that is, with $Y_0=0$);
\item the Jacobian matrix of $I_p(\mN)$ and $I_p(\mM)$ with
...
$$x_{n-s+1}=\mathfrak{h}_{n-s+1,\bu}(1,x_1\dots,x_{n-s}),\dots,x_{n}=\mathfrak{h}_{n,\bu}(1,x_1,\dots,x_{n-s}),$$
and $\mL^H_\bu$ has rank less than $p$ at  $\tilde \bx'=(1,x_1,\dots,x_{n-1})$.
The second item of Lemma~\ref{lemma:appendix} shows that
the Jacobian matrix of $I_p(\mL^H_\bu)$
has rank $(n-s)+1$ at  $\tilde \bx'$.
...
\begin{lemma}
  ...
  such that $\lambda^H_{i_1,i_1}(\bx') = \cdots = \lambda^H_{i_\kappa,i_\kappa}(\bx') = 0$
  and $\mL^H_{\bi;(p+1,\dots,q)}(\bx')$ has rank less than $\kappa$.
\end{lemma}
\begin{proof}
  If $\mL^H_\bu(\bx')$ has rank less than~$p$, then some diagonal
  terms, say $\lambda^H_{i_1,i_1},\dots,\lambda^H_{i_\kappa,i_\kappa}$, must
  ...
  $\lambda^H_{i,i,\bu}$ are random, any system made of more than $n-s$ of
