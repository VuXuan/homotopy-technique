...
evaluation of the matrix $\mF$ has rank less than $p$. Observe that
$S_\mF$ is an algebraic set, since it is defined by the vanishing of
all maximal minors of $\mF$.  In the particular case $p=1$, this
simply means finding all common solutions of $f_{1,1},\dots,f_{1,q}$.
...
sets defined over $\KK$. In the sequel, we also use the classical notion of
...
are called {\em equidimensional}. We refer to e.g.~\cite{Shafarevich77}
for these notions.
...
have dimension at least (and, expectedly, equal to) $n-(q-p+1)$; in
the case $n = q-p+1$, $S_\mF$ is finite for a generic choice of the
entries of $\mF$ (we will make this statement more precise in the
following sections). However, beyond the equality $n = q-p+1$, we make
no assumption on the input matrix $\mF$.

Because $S_\mF$ is an algebraic set, its {\em isolated points} form a
finite subset of $\KKbar{}^n$, defined over $\KK$: these are the
points in the irreducible components of $S_\mF$ of dimension $0$. This
...
\end{itemize}
the constraint on $\lambda$ says that the root of $w$ are the values
...
in~\cite{Schost03,DaSc04}. (The same phenomenom holds with $\KK=k(T)$,
...
$\cdeg(\mF,j)=\max(\deg(f_{i,j}))_{1 \le i \le p}$. 
...
For any ideal $I$ in $\KK[X_1,\dots,X_n]$, $V(I)$ will denote the zero-set of $I$ in
$\KKbar{}^n$. As $V(I)$ is the union of irreducible components, $I$ is
the intersection of finitely many primary components $Q_1\cap\cdots
\cap Q_s$ and this decomposition is said to be minimal when
$V(Q_i)\neq V(Q_j)$ for $i\neq j$. Observe that, for $\bx \in V(I)$
isolated, there exists a unique primary component (from a minimal
primary decomposition of $I$) of dimension zero such that $\bx \in
V(Q_i)$. Then, the \emph{multiplicity} of $\bx$ is defined as the
dimension of $\KK[X_1,\dots,X_n]/Q_i$. We refer to \cite{Eisenbud95} for more
details on these notions.

\smallskip

Finally, for $k \ge 0$, $E_k(\delta_1,\dots,\delta_q)$ is the elementary
symmetric polynomial of degree $k$ in $(\delta_1, \ldots, \delta_q)$
and $S_k(\alpha_1,\dots,\alpha_p)$ is the $k$th complete symmetric
polynomial in $(\alpha_1,\dots,\alpha_p)$.  Our first result is then
the following.
...
%% $$\softO(c^5 m n^2  + c(e+c^5 n)(E + n^3))
