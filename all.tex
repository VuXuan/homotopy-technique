...
\item[${\assD}_2$.] Given $\bx \in V(\bB_0)$ having coordinates in a
  field extension $\LL$ of $\KK$, we can find in time $\softO(D
  [\LL:\KK])$ a sequence $\bi_\bx=(i_1,\dots,i_n)$, with $1 \le i_1 <
  \dots < i_n \le m$, such that the Jacobian matrix of
  $(b_{0,i_1},\dots,b_{0,{i_n}})$ has full rank $n$ at $\bx$, for some
  $D$ independent of $\bx$.
\item[${\assD}_3$.] We know an integer $e$ such that the union of the
  one-dimensional components of $V(J)$ in $\KKbar{}^{n+1}$ has degree at most $e$.
...
we can take
$\bi_\bx=\bi_j$ for all $\bx$ in $Z(\scrR_{0,j})$ (see property $\assD_2$ for 
the definition of $\bi_\bx$). In other words, for
all points described by $\scrR_{0,j}$, the Jacobian matrix of $(b_{0,i})_{i
