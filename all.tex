...
(except when $\ell^H_{i,j}$ is explicitly set to zero in the first
case), that is, $\ell^H_{i,j}=\prod_{k=1}^{\alpha}
\lambda^H_{i,j,k}$. In both cases, the number $N$ is given by
$N=q-p+1$.
