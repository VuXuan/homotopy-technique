...
\]
In addition, if we can prove that there exists a fuction $N(t)$ such that $$
...
\label{subsec:row2}
In this section, we will find a $(p-1)\times (p-1)$ submatrix of $H$, namely $\bar{H}$, such that $m(\mathbf{x}^*) \ne 0$, where $m = \det(\bar{H})$ and $\mathbf{x}^*$ is a solution of $\g$ when we work on the case $\deg(f_{i,j}) \leq D_i$ for all $1\leq j \leq q$. Let us recall the starting matrix $G$ in this case is 
\[ G = \left( \begin{matrix}
...
Labex Milyon for giving me a scholarship to come to ENS Lyon for studying. I acknowledge financial support provided by the scholarship \emph{Explora Doc} from \emph{Rêgion Rhône-Alpes, France}.  
...
Let ${\bf x}$ be a solution of the system $g_1^H(0,\mat{X}) = \cdots = g_M^H(0,\mat{X})$. We would like to prove ${\bf x} = (0,\ldots, 0)$. Let us assume there exists one coordinate of ${\bf x} = (x_1, \ldots, x_n)$ is non-zero. Without loss of generily, let us assume that $x_n = 0$ and $L_1^H(0,{\bf x}) = \cdots = L_k^H(0,{\bf x}) = 0$, where $L_i$ is a linear form of $\prod_{t=1}^{D_i}L_i^{(t)}$ and $k \leq \min(n,p)$. Let us denote $A$ for the submatrix of $G$ that contains first $k$ rows and columns $p+1, \ldots, q$ of $G$, that is, $A = G_{1:k;p+1:q}$. Let ${\bf a} = (a_1, \ldots, a_m)$ be the system of all $k \times k$ minors of $A$, then ${\bf a}^H(0,{\bf x}) = 0$. From $L_1^H(0,{\bf X}) = \cdots = L_k^H({\bf X}) = 0$, we can rewrite $\{X_i\}_{i=1}^k$ in the form $\sum_{j = k+1}^n \ell_{i,j}(\boldsymbol{\cdot})X_j$, 
where $\ell_{i,j}(\boldsymbol{\cdot})$ are fuctions in generic coefficients; and then substitute $\{X_i\}_{i=1}^k$ into $A$. The matrix $A$ now is in dimension $k \times (q-p)$ of $n-k$ variables $X_{k+1}, \ldots, X_n$. Since $g_{i,j}$ is a product of $D_i$ linear form, then all of $a_i^H(0,{\bf X})$ is a homogeneous polynomial in the form 
\begin{equation}
