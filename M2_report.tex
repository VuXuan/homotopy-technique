...
Now, we move to the main problem in this report which is: given a multivariate polynomial matrix $F \in \field[\mat{X}]^{p \times q}$ with the number of variables is $q-p+1$ and $p \leq q$, compute the isolated solutions of the determinantal system made by all $p \times p$ minors of $F$. We study two cases for the input matrix, that are either column degrees and row degrees. We recall here that the notation $\mathcal{T}$ is either $E_{q-p+1}(D_1, \ldots, D_q)$ in the case of column degrees or $S_{q-p+1}(D_1, \ldots, D_p)$ in the row degrees case. Notice that in this report, we are working on the case when $n = q-p+1$. Before going to details of the start matrix, we need some genericity assumptions for the case of row degrees. 
\subsection{Genericity assumptions}
Let $G = [g_{i,j}]_{1 \leq i \leq p, 1 \leq j \leq q}$ be a matrix with $g_{i,j}$ is a product of $D_i$ linear forms with each linear form has generic coefficients. Let $I_G$ be the ideal generated by all $p \times p$ minors of $G$. We say that the matrix $G$ satisfies assumption $\mathcal{A}$ if 
\begin{enumerate}
...
In order to use the symbolic homotopy teachniques, we need a start matrix $G$ corresponding to the target matrix $F$ with some properties which we have mentioned in the \improve{\cref{sec:intro}}. 
\subsubsection{Column degrees}
\label{subsec:cd}
We first consider the case when $F = [f_{i,j}] \in \field[\mat{X}]^{p\times q}$ with $\deg({f_{i,j}}) = D_j$ for all $1 \leq i \leq p$. We will construct a polynomial matrix $G \in \field[\mat{X}]^{p \times q}$ such that the determinantal system of $G$ has $E_{n}(D_1, \ldots, D_q)$ solutionss and we can find those solutions in effectively way. 
...
\begin{Lemma} \label{G} Let G be the matrix as we define above and $I_G$ be the ideal generated by $p \times p$ minors of $G$. Then, $I_G$ has exactly $E_{n}(D_1, \ldots, D_q)$ solutions and there is an algorithm, namely Algorithm \ref{StartMatCol}, to compute these solutions in $\bigO{nE_{n}(D_1, \ldots, D_q)}$ operations in $\field$.
\end{Lemma}
\begin{proof}
...
\] Then, for any solution $\mathbf{x}$ of $I_G$, $\mathbf{x}$ is a solution of a $n$ polynomials system which is taken from $\{g_j\}_{1 \leq j \leq q}$. This implies 
\begin{eqnarray*}
\#\{\mathrm{solutions \ of \ } I_G \} &=& \sum_{(i_1, \ldots, i_{n}) \subset \{1, \ldots,q\}^{n}} \#
...
{\bf Output}: $E_{n}(D_1, \ldots, D_q)$ solutions of $I_G$. 
...
\caption{StartMatrixColumn}
\label{StartMatCol}
\end{algorithm}
Furthermore, by the construction of $G$ as above, we can see that there are no solution of $I_G$ at infinity. 
...
\[ G = \left( \begin{matrix}
g_{1,1} & 0 & \cdots & 0 & g_{1,p+1} & \cdots & g_{1, q}\\
0 & g_{2,2} & \cdots & 0 & g_{2,p+1} & \cdots & g_{2, q}\\
\vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & g_{p,p} & g_{p,p+1} & \cdots & g_{p, q}
\end{matrix} \right), \] where all of $\{g_{i,j}\}$ is a product of $D_i$ linear form with generic coefficients. That is $g_{i,j} = \prod_{k=1}^{D_i}L_{i,j}^{(k)}$, where $L_{i,j}^{(k)}$ is in liner form with generic coefficents. 

Let $\mathbf{x}$ be a solution of $I_G$, the next property help us reduce our problem to the problem with smaller dimension. 
\begin{Proposition} \label{r2}
If $g_{i_1,i_1}(\mathbf{x}) = \cdots = g_{i_k,i_k}(\mathbf{x}) = 0$, then $\mathrm{rank}(G_{i_1:i_k\mathbf{;}p+1:q}({\mathbf{x}})) \leq k-1$.
 %for $(i_1, \ldots, i_k) \in \{1, \ldots, p\}^k$
\end{Proposition}
\begin{proof} Without of loss of generality, we prove this result for $(i_1, \ldots, i_k) = (1, \ldots, k)$. This means we have $g_{1,1}(\mathbf{x}) = \cdots = g_{k,k}(\mathbf{x}) = 0$ and $g_{j,j}(\mathbf{x}) \ne 0$ for any $j \in \{k+1, \ldots, p\}$. Let $G_{1:p;*} \in \field[\mat{X}]^{p \times p}$ be the submatrix of $G$ consisting $p$ rows and the columns $k+1, \ldots, p, j_1, \ldots, j_k$ of $G$, where $(j_1, \ldots, j_k) \in \{p+1, \ldots, q\}^{k}$. Let $f_{1:p;*}$ is the determinant of $G_{1:p;*}$. Then, for any solution of $I_G$, $\mathbf{x}$, $g_{1:p;*}(\mathbf{x}) = 0$. Moreover, $g_{1:p;*} = \det(G_{1:p;*}) = g_{k+1,k+1} \ldots g_{p,p} \det(G_{1:k;j_1:j_k})$. Therefore, $\det(G_{1:k;j_1:j_k})(\mathbf{x}) = 0$. This holds for any $(j_1, \ldots, j_k) \in \{p+1, \ldots, q\}^{k}$. This implies that $\mathrm{rank}(G_{i_1:i_k\mathbf{;}p+1:q}({\mathbf{x}})) \leq k-1$. 
\end{proof}
\begin{Lemma}\label{P3} Let G be the matrix as we define above and $I_G$ be the ideal generated by $p \times p$ minors of $G$. Then, $I_G$ has exactly $S_{n}(D_1, \ldots, D_p)$ solutions. Moreover, there is an algorithm, namely Algorithm \ref{StartMatRow}, to compute these solutions in \todo{Complexity}. 
\end{Lemma}
\begin{proof}
...
\fbox{ \begin{minipage}{13.5cm}
{\bf Input}: a matrix $G \in \field[\mat{X}]^{p \times q}$ which is defined as above.\\
{\bf Output}: $S_{n}(D_1, \ldots, D_p)$ solutions of $I_G$. 
%\begin{enumerate}
...
\begin{itemize}
\item[•] If $p \leq q$:\\ \hspace{1cm} $\mathbf{x} \gets$ \todo{RowHomDet}($G_{i_1:i_k\mathbf{;}p+1:q}, G_{i_1:i_k;i_1:i_k \cup i_{n}:i_{n+k-1}}$)
\item[•] Else :\\ \quad $\mathbf{x} \gets$ \todo{ColHomDet}($G_{i_1:i_k\mathbf{;}p+1:q},\bar{G}$), where 
...
\caption{StartMatrixRow}
\label{StartMatRow}
\end{algorithm}

\subsection{A bound for the number isolated solutions of $I_F$}
Let $\mathcal{T}$ be the number of solutions of $I_G$. That is $\mathcal{T} = E_{n}(D_1, \ldots, D_q)$ in the column degrees case, and $\mathcal{T} = S_{n}(D_1, \ldots, D_p)$ in the row degrees case. Let us define $H := (1-t)G + tF$. We also write $I_F := \langle p \times p - \mathrm{minors \ of \ } F\rangle$. In this section, we are going to prove that the number of isolated solutions of $I_F$ is at most $\mathcal{T}$. We first recall here the result in \cite[Proposition~1]{Hen83}. 
...
\]
On one hand, since the total degree of $h_{\mat{X}}(t, \mat{X})$ is at most $D$, then \[\nu(h(t, f_1(t), \ldots, f_n(t))t^{Dd}) \ge 0\] This means $\nu(t.h(t, f_1(t), \ldots, f_n(t))t^{Dd}) > 0$. On the other hand, we claim that \[\nu(f_G^{\mathrm{Hom}}(t^d, s_1(t), \ldots, s_n(t))) = 0.\] Indeed, it is easy to see that $\nu(f_G^{\mathrm{Hom}}(t^d, s_1(t), \ldots, s_n(t))) \ge 0$. Furthermore, we claim that $f_G^{\mathrm{Hom}}(0, s_1(0), \ldots, s_n(0)) \ne 0$. Then, as a consequence,  
\[
(1-t)^pf_G^{\mathrm{Hom}}(t^d, s_1(t), \ldots, s_n(t))  + t.h(t, f_1(t), \ldots, f_n(t))t^{Dd} \ne 0
\]
 which is a constradiction. Thus, the remaing problem we need to prove is $f_G^{\mathrm{Hom}}(0, s_1(0), \ldots, s_n(0))$ is indeed different zero. Assume a contradiction that $f_G^{\mathrm{Hom}}(0, s_1(0), \ldots, s_n(0)) = 0$.
\begin{enumerate}
\item[(1).] $\mathbf{Column \ degrees \ case}$: Since any $f_G \in I_G$, $f_G$ has the form $f_G = \lambda. g_{i_1} \ldots g_{i_p}$ where $(i_1, \ldots, i_p)\subset \{1, \ldots, q\}^p$ and $\lambda \in \field$, then 
\[
...
\item[(2).] $\mathbf{Row \ degrees \ case}$: \todo{todo: finish the proof}
\end{enumerate}
\end{proof}
\begin{Proposition}
...
\end{Proposition}
\begin{proof}
We first remark that when $t = 0$, we have $V(I_H) = V(I_G)$. For the column degrees case, the result can be obained directly from the construction of the start matrix $G$ in the \improve{\cref{subsec:cd}}. For the 
\end{proof}
